i think this is old... not sure what replaces it

- todo: 
-- turn errors off in production
-- verify CSRF tags
-- VALIDATE all input data.  scrub all user input data (and at output
-- patch levels on (
-- own php server for front page
-- turn off forced directory browsing
-- use SSL everywhere
?? is our HT ACCESS done using SSL
?? cyber liability insurance
-- signup for Borski beta signup list

- Later
-- Are we using POSTS everywhere we should be
-- use firewall to ensure production server can only go DB server
-- instead of ROBOTS.txt use access controls (limit it to IPs, or PW)
-- look at what users have access to the DB
-- veify type and length of all user input
...
### CureBit -- they connected to many shopping carts
...
### STARTUP MARKETING -- Jared from Away Find
...
### 11.10.20 -- MVP discussion w. frank
...
### 11.10.18 -- One line pitch
We're a group payments solution where the organizer is never on the 
hook for group purchases like vacation rentals, sporting events, concerts, 
and other purchase. We integrated directly with merchants as a checkout button. 
======
...
funny thing... here is startup-land Mountain View style, nobody could give two S**ts about DARPA or being a PM.
the question is what have you _DONE_     (giving other people money, just doesn't even rate as 'doing' ...), and
what can you _DO_ right now.... like in the next five minutes, right now...
...
but fun .... 
 ...
 my CEO co-founder is now market-testing a product I have not built yet... in a language (rails) I don't know yet, on a deployment platform (Amazon EC2) that I have never used before, incorporating dozens of tools/languages/api/payment platforms, etc that I have yet to learn.

30 days till product launch...  and it cannot be flaky, must be online upgradable, since it is primary purpose is a payment processing platform.
12.5 weeks till demo day.... 


So, Danny, what can you do NOW?   
 
I don't believe it is possible to have undergone a more rapid downgrade in authority and status.
(but oddly very satisfying to have everything ride on what you produce w. your own fingers)
