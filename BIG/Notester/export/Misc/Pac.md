# Misc.Pac --
    ...
    Assume a universe of points XX contained in all distributions, D. 
    The function D(X) is the probability of the point X in XX according to  
    distribution D.  (D(X)=0 iff X is not in distribution D.) 
     
     
    Defintion:  The movement between two distributions D_1 and D_2 is  
    simply the sum of the difference bewteen the two distributions: 
       Movement(D_1, D_2) = SUM_(X in XX)  D_1(X) - D_2(X) 
     
    The Movement Lemma:   
    For any partition region Pi 
      if Movement(D_1, D_2) < some-poly-of-epsilon then 
         | Tightnes_D_1(Pi) - Tightness_D_2(Pi) | < epsilon 
     
     
    (NOTE: I will have to push through the proof to know exacltly what 
           goes in the "some-poly-of-epsilon" spot) 
