# Original\_Forum\_Info\_File --
    ...
    * Menu:
    ...
    Methods for overlaying this structure onto existing hierarchies

    Goal:  Somehow splice efforts of ongoing ontologies into our hierarchy
    ...
    * Menu:                                 
    ...
    The forum is an electronic mechanism for debate and consensus through
    discussion.


    OBJECTIVES

    The forum seeks to express our current understanding of issues of
    importance.  It is an electronic discussion that encourages fair
    debate followed by consensus building.

    It seeks to encourage 

    Goals:
    - Consensus.  Facilitate formation of consensus opinion.
    ...
    Why should people want to do it?
    Want to be heard.  Want to contribute.  They know they are right.
    ...
    * Menu:                                 
    ...
    SUBMISSION
    Participants continuously submit notes, revisions, and retractions.


    The issues taxonomy is built by recursively selecting the summary 
    with greatest support and least contention.  This taxonomy extends 
    down to the points of contention.  
    (What about major<<>>minor points not in summary?)

    Summaries for points of contention are selected by support.
    They must include all major and minor positions.  If
    great contention exists over the best summary, moderators, may select
    one, or allow alternatives to be arranged.

    Positions are ranked based on the support provided to them as major, 
    minor, or proposed.  Each participant may support at most one position 
    for a point.  Only after they have thrown their support behind a position
    can they support or oppose its justification sub-structure.

    Summaries can be written for 
    ...
    - Varient issue wordings that are most widely supported and least contentions
      are endorsed.  (
    ...
    VARIENTS
    A set of nodes with the same parent may be declared varients.
    This declaration may be supported or opposed by the participants.


    AUTHORING 
    Any participant may submit an Issue, Point, Position, or Opinion.

    RENDERING SUPPORT
    Each participant may support or oppose Issues, Points, Positions, or Opinions.
    A note of support (or opposition) may optionally include a justification
    for the opinion.
    ...
    Cross listing -- Because of varients in the tree, cross-listing
    a node under multiple parents should be allowed, but support
    should be afforded such and entry only if the participant is satisfied
    with its placement under *all* parents.
    ...
    * Menu:                                 
    ...
    VOLUME
         Traffic per unit time at a particular point in time.
    ...
    * Menu:                                 
    ...
    
    Node: conventions, Up: Top
    * Menu:                                 

    The conventions for discourse are the rules that governing how the
    XXXX proceeded.  These convensions include specifications of (1) the
    mechanical procedures followed by the system, (2) specifications
    regarding the display of the information, and (3) guidelines for
    appropriate participant behavior.


    Conventions are set for a particular node and are inherited 
    through the hierarchy.

    > Conventions should be explicit, but should evolve over time.
    > Conventions should be cover wide area (so no abuse)

    > How to select and update convensions?

    > Convensions accumulate (amend) down the tree, and can be added only
      by a consensus at a node.
    ...
    * Menu:                                 
    ...
    
    Node: using-usenet, Up: Top
    * Menu:                                 

    How to integrate approach with existing USENET groups

    Goals:
    - make larger community aware of the CONSENSUS net
    - don't bother plain usenet community
    - convenient to do both usenet and consensus at the same time




    Design Points
    * Nodes can be associated with newsgroup.  Node contributors are
      prompted to ask if they should also be posted.

    * Posts can also serve as a consensus entry.
