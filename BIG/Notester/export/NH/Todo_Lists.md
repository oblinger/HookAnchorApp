# NH.Todo\_Lists --
    ...
    TODO LIST INVARIENTS    (change containingProject list to getParent)
    * Every todo list has a todo list note (this is its project list) or the orphanage
    * Throwing will link item to new list.  It is removed from the old list if:
       the new/old lists are global or the new list is not global and old list was parent.

    DISOWN -> 
      Removed from the current global/project list (must be on another list)
      Parent changed to GlobalItemsList if removed from its own parent 
    DELETED ->        (think about this part)
       - archive
       - place on a 'X-ed' global list
       - if Xing & on project list then leave on project list
       - if nonTrivial then move aside
       - Place on 'Deleted' project list
