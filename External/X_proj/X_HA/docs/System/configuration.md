...
Controls keyboard shortcuts for all actions. Keys can be specified as:

#### Key Format Types
- **Single keys**: `"Escape"`, `"Enter"`, `"Space"`, `"Delete"`, `"Tab"`
- **Arrow keys**: `"ArrowUp"`, `"ArrowDown"`, `"ArrowLeft"`, `"ArrowRight"`
- **Letters**: `"a"`, `"b"`, `"c"` (case-insensitive, normalized to lowercase)
- **Numbers**: `"1"`, `"2"`, `"3"`
- **Punctuation**: `"/"`, `"="`, `";"`, `"?"` (use ASCII form)
- **Special**: `"Backtick"`, `"Plus"`, `"Minus"`, `"Equals"`, `"Slash"`
...
- `"alt+enter"` - Alt+Enter
- `"cmd+shift+plus"` - Command+Shift+Plus

#### Shifted Punctuation Handling
The system automatically handles keyboard variations for shifted punctuation:
- `"+"` accepts both `Equals+Shift` and `Plus` key events
- `"?"` accepts both `Slash+Shift` and `Questionmark` key events
- `"!"` accepts both `1+Shift` and `Exclamation` key events

This ensures consistent behavior across different keyboard layouts.

#### Default Key Bindings
| Action | Default Key | Description |
|--------|-------------|-------------|
| exit_app | `Escape` | Close popup window |
...
| force_rebuild | `` ` `` | Force rebuild and rescan |
| show_folder | `/` | Open folder of selected command |
| open_editor | `=` | Open command editor |
| edit_active_command | `;` | Edit currently selected command |
| show_keys | `?` | Display all key bindings |
| delete_command | `Delete` | Remove selected command |

#### Template Keys
Templates can define their own trigger keys:
```yaml
templates:
  my_template:
    key: "t"  # Triggered with 't' key
    # ... template definition
```

#### Vim Mode Support (Future)
When vim_mode is enabled, additional navigation keys become available:
- `h` → Move left
- `j` → Move down  
- `k` → Move up
- `l` → Move right
