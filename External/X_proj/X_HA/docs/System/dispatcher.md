...
### Registration (v0.8.0+)
**Important**: The dispatcher (`ha`) does NOT register URLs directly. URL registration is handled by the separate URLHandler.app embedded in the main application bundle to prevent system hangs.

```
URLHandler.app (in Resources/) → Registers hook:// → Calls: ha --hook <url>
```

### URL Processing
When invoked with `--hook` option:
...
fn handle_hook_url(url: &str) {
    ...
    // Execute directly without showing popup
...
**Note**: The URL handler must exit quickly (< 100ms) to prevent system hangs. See [URL_HANDLER_ARCHITECTURE.md](URL_HANDLER_ARCHITECTURE.md) for critical safety information.
