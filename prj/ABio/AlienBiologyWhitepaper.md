...
We believe the first kind of learning and inference is well measured by existing agentic testing.  In this case, we *want* the agent to be exposed to static written forms and then measure the performance that results.  Alien biology is not trying to test type A. Indeed, we hope to be pretty isolated from type A knowledge and instead are focused on type B learning and inference.  Type C is out of scope for this first version of Alien Biology.
