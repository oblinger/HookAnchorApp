# Top
#### kind: StartAI
#### area: CoreDL
#### stance: 
## Sections
### Cash
#### cash_trend: flat
#### cash: 
#### cash_available: 7
#### funding: 
Added $12.2M in Series A-II funding (May 2023, led by Felicis) ￼ (total funding ~$28M)
### Coolness
#### cool: 
#### cool_why: 
#### cool_mission: 
#### cool_balance: 
#### cool_dan: 
#### mission: 
To make it dead simple for novices and experts alike to build ML applications and get them into production with just a few lines of code ￼.
### Deep Learning Projects (DLP)
#### dlp: 
#### dlproj1: 
Ludwig v0.8 – an open-source declarative ML framework (originated by Predibase) now optimized for generative AI, allowing users to fine-tune LLMs on private data via simple configs, with built-in support for techniques like LoRA, QLoRA, DeepSpeed, and prompt templating ￼ ￼.
#### dlproj2: 
Predibase Platform (GenAI) – a low-code ML platform that lets teams declaratively build, fine-tune, and deploy large language models using their proprietary data, featuring an AI copilot (“LudwigGPT”) for guidance and enterprise-grade controls ￼ ￼.
#### dlproj3: 
“LudwigGPT” Copilot – Predibase’s custom foundation model powering its data science assistant, which provides real-time suggestions and improvements as users develop models, illustrating the platform’s use of generative AI to enhance ML workflows ￼.
### Job Postings
#### job: 
#### job_why: 
#### job_bay: Software Engineer – ML Inference – Google Search​
#### job_remote: None currently
### Toxicity
## Misc Fields
#### bam:
#### dlp_:
