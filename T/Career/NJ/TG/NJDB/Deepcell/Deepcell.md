.[[Deepcell]].
  , ,
  , ,
  , ,
  , [Deepcell Obsidian](spot://deepcellobsidian),
  DELS: ,[Deepcell Obsidian](spot://deepcellobsidian),
  , ,
  , [Deepcell Note](spot://deepcellnote), 
  DELS: [Deepcell Note](spot://deepcellnote),[Deepcell Obsidian](spot://deepcellobsidian),[Deepcell Note](spot://deepcellnote),[Deepcell Note](spot://deepcellnote),

# Top
#### kind: StartAI
#### area: Bio
#### stance: 
## Sections
### Cash
#### cash_trend: upward
#### cash: 
#### cash_available: 24
### Coolness
#### cool: B+
#### cool_why: 
#### cool_mission: 
#### cool_balance: 
#### cool_dan: 
#### mission: 
Deepcell is pioneering new methods in single-cell analysis by combining innovations in microfluidics, optics, and AI ￼.
### Deep Learning Projects (DLP)
#### dlp: B ?
#### dlproj1: 
Human Foundation Model & Morphology Atlas: Deepcell’s foundation AI model trained on millions of cell images enables label-free classification of single cells by morphology; used to build a “Deep Cell Atlas” with over 1 billion cell images ￼ ￼.
#### dlproj2: 
REM-I platform for morpholomics: AI-powered benchtop system (launched 2023) combining imaging, microfluidics, and deep learning (Human Foundation Model) to analyze and sort single cells based on high-dimensional morphology features ￼.
#### dlproj3: 
Tabula Sapiens cell atlas project: Collaboration with Stanford (Chan Zuckerberg Biohub) to generate and publicly share single-cell morphology data for a human cell atlas of 2 million cells, using Deepcell’s AI platform to identify cell types via morphology ￼ ￼.
### Job Postings
#### job: 
#### job_why: 
#### job_bay: VP, Data Science & AI – Google search
#### job_remote: (No remote openings found requiring AI experience & leadership)
### Toxicity
## Misc Fields
#### bam:
#### dlp_:
### recent_funding: Secured $73M Series B in Mar 2022 ￼
### sdfsdfsfsfds: foo
