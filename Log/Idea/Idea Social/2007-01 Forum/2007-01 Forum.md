.[[Forum]].
  [PPTX](hook://forumpptx) 
  [[2021-01-00 Short start with Yashin]], [[Truth]],

[[POLIS]] 


http://localhost:4567
http://localhost:4567/admin      admin   admin123
...
## 2025-09-19  Sketch of a way forward


- Use a dialectic structure where at each node one has a best rendition of a position and a best rendition of the rebuttal for that position.
- Each of these are voted on by the respective groups (in and out)
- Each are built from parts that are authored and upvoted
- Special "redundancy" votes indicates strongly performing parts and are better covered by other strong parts.
- Best set is identified and best author(s) are selected to provide consensus position / rebuttal
...
Synthea
  ...
  Names Emphasizing Tribal/Political Dynamics:

  - The Tribalist - Embraces the tribal nature of political discourse
  - TribeSpeak - Where political tribes engage
  - CrossTribal - Emphasizes bridge-building across tribes
  - TribalBridge - Connecting political tribes

  Names Emphasizing Synthesis/Consensus:

  - SynthesisPoint - Finding synthesis in political debate
  - CommonGround - Discovering shared positions
  - BridgePoint - Building bridges between positions
  - ConsensusTree - Growing consensus through argument trees

  Names Emphasizing Constructive Discourse:

  - CivicTree - Growing civic discourse
  - ArgumentForge - Forging stronger arguments together
  - DebateWorks - Making debate productive
  - PositionSpace - Mapping political positions

  Names Emphasizing Democratic Process:

  - VoxPolis - Voice of the political community
  - DemosHub - Center for democratic discourse
  - CitizenDebate - Citizen-driven political discussion
  - PublicSquare - Digital public square for politics


## 2025-09-19  
 We have a very specialized way that we present a note to the user both the viewer of the note, and also the editor of the note. When we're showing a note off to the right hand
  side, we have a number of comments that connect with particular parts of the note itself. So the display looks a little bit like track changesin an editor, but it's on a
  single note, and this display is very dynamically generated. I want to have a version of this viewer in the newsfeed of unread messages. But also when they go into the
  detailed tree viewer, they will see this kind of display as well and as they click around in that display, they'll actually navigate the tree and be looking at other notes. So
  my question is should we plan to have a Note viewerwhich is completely outside of the BB architecture, or should we create a plug-in for no BB that views notes in the
  stylized way? What are the pros and cons?


## Next Steps

- Define position statement to topic mapping strategy
- Design argument tree storage within NodeBB post structure
- Plan support/oppose voting implementation
- Architect tribal affiliation derivation system

## 2025-09-13  


  1. Tribal Affiliation System
    - How do users select their political affiliation? (Progressive, Conservative, Libertarian, Centrist from your .env)
    - Should this be mandatory or optional during registration?
    - Can users change affiliations later?
  2. Discussion Features
    - Cross-tribal discussion rooms?
    - Argument synthesis tools - what exactly should these do?
    - Moderation enhancements specific to political discourse?
  3. Plugin Architecture
    - Should this be one comprehensive plugin or multiple smaller plugins?
    - What NodeBB hooks do we need to integrate with?
    - Any special UI components needed?
  4. Data Structure
    - How do we store tribal affiliations?
    - What additional user metadata do we need?
    - Any special content categorization?


  Potential Integration Strategies:

  Option 1: Map to NodeBB Native Structure
  - Categories = Political Issues
  - Topics = Position Statements
  - Posts = Arguments/Rebuttals
  - Pros: Uses NodeBB as intended
  - Cons: Loses argument hierarchy, no native support/oppose

  Option 2: Custom Plugin with Data Layer
  - Store argument trees in separate MongoDB collections
  - Use NodeBB categories/topics as "containers"
  - Plugin renders custom UI for hierarchical arguments
  - Pros: Preserves your design, leverages NodeBB infrastructure
  - Cons: Complex plugin development

  Option 3: Hybrid Approach
  - NodeBB categories for organization
  - Custom post types for positions vs. arguments
  - Plugin adds support/oppose metadata to posts
  - Custom rendering for tree view
  - Pros: Balances complexity with functionality

  Questions for you:
  1. How important is preserving the exact hierarchical structure?
  2. Should we try to make it look/feel like traditional NodeBB, or can it be completely custom UI?
  3. Do we need NodeBB's existing features (search, notifications, user management) or mainly the infrastructure?


## 2025-09-11  Testing Framework Description

I want to rapidly prototype a framework that is designed to allow a collection of people to with diverse political viewpoints to collaborate and compete to synthesize a number of position statements on various issues as well as arguments about details in those positions and counter arguments to those arguments. My aim is to quickly build a test framework that I can use with 20 to 100 participants that will spend several hours per week reading threaded discussions around topics that interest them and comment on and refine the arguments being made by others within this form.

The arguments themselves are probably presented on a webpage on a larger device like a tablet or a laptop viewing the result. Each page might describe a single position on a topic and then in the margin there's comments almost like you have comments in a Google doc where the comments refer to the text itself. But the idea is that the participants are grouped into liberal and conservative, and if a liberal posts a position on a topic, then other liberals might propose amendments or edits to that topic while conservatives would indicate support are usually anti-support for specific parts of that position if there's various claims made in the position they might try to refute those claims. All of this is 100% mechanical so it has to do with indicating support or anti-support or edits on each note in this outline or tree of topics and points encounter points.

The main view is a text view on a particular bit of text which is a claim or a position or an issue statement and then off to the right hand side with arrows kind of indicating the text would be comments or rebuttals to the text there and each one of these rebuttals often would have a rebuttal to it, which was also viewable. The full system would be one that allows you to move your mouse over each one of these sub points and then the tree of commentary around that would come into view. But the idea is instead of seeing all the comments made at any given note in the tree. You really just seemed the best comments. So the idea is you have the best version of some text which is presented and then there's alternative version that you can look at, but you're only presented with one and then with the one that you're presented there's a bunch of rebuttals to it, but you're only presented with the best rebuttal to it and for each rebottle people who are opposed to this position can actually propose alternative rebuttals for each one alternative statements of it so overtime hopefully we get the best framing of a position and the best framing of the top challenges to that position and the best challenges to those challenges.

In order to quickly build such a system, one thought is to begin from a threaded discussion forum. Such systems already give you authentication user IDs for your different users the ability to post and to comment on other posts in a threaded discussion. So one idea is t one uses the threaded discussion tool to manage a threaded discussion and then we either modify the tool to allow the addition of some Mehta data as you are adding your additional information and to say whether you're supporting or refuting a point or whether you're proposing an alternative writeup of the same point. Or maybe in the subject line you simply use plus one minus one just as a poor man's way of indicating the information.

Either way, my font is that's a static bit of code could scan the entire contents of this threaded discussion forum assuming it's in a sequel database such and from that automatically derive the human viewable cross-linked set of issues and positions and arguments and such. This piece would be written from scratch in order to get the correct look and feel of the text. Discussion with highlights in it.

The parts of this that are missing is for the simplest version of this we might just allow users to simply plus one some no or minus one the node or equal equal the node which means I'm giving you an alternative for it and there's no editing of anything. Each person that authors, a message presuming to go back in and edit that message but that would be it. Slightly more advanced version of this would allow others to propose edits, which could be accepted or not by the original author, on the note, this would require a tool that allows you to specify text edits a little bit like version control in Microsoft Word or you show the edits themselves and then the author could accept the changes or not. That probably requires quite a bit more specialized tooling to do that.

Given this basic system set up, what components do you think will be the best starting point for this? What technologies would you build such a system out of and then give me a list of the major components that needs to get built in order to develop this system. In short, give me a design document for how you would go about building such a system, with an eye towards minimizing the work it would take to build the first version of the system.  Please put this design into 001 System Design.md


## 2025-09-11  Cameron Peltz - Civic Tech DC

215 840 1400
redit 

Pointers:
- Subreddit of folks who would participate.
- Arnold Kling's Three tribal coalitions [Book](https://www.cato.org/three-languages-of-politics) 
- [NoLabels](https://nolabels.org/) bi-partisan group
...
[https://otter.ai/u/KNndSgWqDOfLecWyL2Irko_Cq38?view=summary](https://otter.ai/u/KNndSgWqDOfLecWyL2Irko_Cq38?view=summary)

Cameron Peltz, a data scientist at the Health and Human Services headquarters, discussed with Daniel Oblinger and Y S about a project aimed at fostering bipartisan discussions. Daniel, with a background in AI and machine learning, proposed a system that rewards informed, balanced opinions and allows users to edit and vote on content. The system, resembling a blend of Wikipedia and Reddit, aims to present diverse viewpoints and moderate extremities. Cameron highlighted the potential for engagement and the importance of trust in sources. They also discussed the challenges of finding conservative groups willing to participate and the need for the system to be compelling and open-minded.

## Action Items

- [ ] @Dan Oblinger - Provide more details on the technical implementation of the platform, including the specific algorithms and mechanisms for curating and presenting content.
- [ ] @Dan Oblinger - Assess the feasibility of finding conservative groups or individuals who would be willing to engage in the platform's debate-style discussions.
- [ ] @Cameron Peltz - Explore potential partnerships with bipartisan or non-partisan organizations in Washington, D.C. that may be interested in the platform concept.

## Outline

### Introductions and Initial Technical Issues

- Cameron Peltz initiates the conversation, checking if others can hear him and mentioning Yashin's inability to hear.
- Daniel Oblinger confirms he can hear Cameron and mentions putting on his headset.
- Y S suggests Cameron introduce himself and mentions knowing Daniel from undergrad.
- Cameron introduces himself as a data scientist at the Health and Human Services headquarters, working on automating internal business intelligence reporting and machine learning.

### Overview of Participants' Backgrounds

- Y S provides a brief introduction of Daniel, mentioning his background in machine learning and their long-standing email discussions.
- Cameron shares his knowledge of organizations in DC that might be interested in the project, mentioning his eight years of living in DC and connections with summer camp programs.
- Daniel provides a quick introduction, mentioning his PhD in AI, experience as a program manager, and current work at Sports Vizio.
- Daniel clarifies that the idea isn't particularly related to machine learning but involves AGI and a career shift from old-school machine learning to new-school machine learning.

### Discussion on Project Goals and Challenges

- Daniel explains the project's goal of finding a conservative group to engage in a debate-style discussion, noting that liberals are generally more willing to engage.
- Daniel shares his hypothesis that people would engage in the mechanism if it worked and that the outputs would be compelling to others.
- Daniel discusses the challenges of finding a conservative group and the importance of having both sides engage in the discussion.
- Daniel presents a slide explaining the idea of having an informed, open-minded expert from the user's tribe speak on complicated issues.

### Explanation of the System's Mechanics

- Daniel describes the system as a self-working system that rewards informed, balanced, and open expert opinions.
- Daniel explains the importance of having people who represent the user's tribe in the system to ensure credibility.
- Daniel compares the system to Wikipedia, where users can edit content and engage in discussions, but with a focus on curated content at the top.
- Daniel provides an example of how the system would work, showing how users can post and edit content, and how the algorithm would track and reward contributions.

### Detailed Explanation of the Algorithm

- Daniel explains the algorithm's role in moderating extremities and ensuring the best case bubbles to the top.
- Daniel describes how users would self-report their positions and how the algorithm would track and reward contributions based on the user's self-identified partisanship.
- Daniel discusses the importance of having a diverse group of editors to ensure the content is balanced and not biased.
- Daniel shares his vision of the system allowing each citizen to find their tribe speaking on issues in a compelling and clear manner.

### Potential Impact and Challenges

- Cameron expresses interest in the idea, noting the potential for people to see across the political spectrum and understand different views.
- Daniel discusses the challenges of getting people to engage and generate valuable content, as well as the question of whether entrenched views would be open to new information.
- Daniel shares his belief that the system could be more effective than traditional media in shifting opinions because it would come from trusted sources.
- Daniel mentions the importance of having a system that is transparent and allows for open and honest discussions.
...
==> the proof at this larger scale is whether un-related partisans would read the partisan publications from this org, and be swayed by their findings, if this was show to be true, its real proof that we have something of large value. — this is the point that funders would care.
...
Steps:
2. High accuracy, disinterested authors are presented with specific adjudications in order to offer an opinion.
3. Both past and future authors with an interest in this topic are presented with the same adjudication for consideration.
...
4. Content is continously adjudicated as it is read.  Any reader is invited to offer constructive feedback on the content they read.  Feedback should be written in a constructive, positive, and as narrow a way as possible even when one is commenting on stances that one strongly disagrees with.
	- When possible feedback should be tied to the phrase, sentence or paragragh to which it applies.
5. Authors are given opportunity to edit their phrasing etc. to "fix" the issue raised... even if they dont fully agree with the criticism if they can make an edit to make their audience "happy" they may do this, just to minimize the number of highlighted issues raised with their content.
6. Each point of feedback on each stance is itself a stance and others may agree or disagree with this feedback itself.  A stance on a stance is called an ADJUDICATION
...
8. Whose biases are mechanically selected to reflect the biases of the population at large, 
9. Who have been shown to render impartial judgements as measured mechanically in comparison with their peers
10. Who show significant engagement with the platform in rendering opinions.
...
11. TOPIC -- Sponsor or community select topics for consideration.

=== COLLABORATION PERIOD ===
12. INITIAL AUTHORING -- Left and Right authors are selected at random from appropriate quality class based on resources provided.
13. PAIRWISE COLLABORATIVE REFINEMENT -- Each pen their own separate analysis, and collaborate with each other to constructively revise their own analysis based on feedback from their counter part.
14. CONSENSUS STATEMENT -- They also work with their counterparts to collectively author a list of consensus points that both accept as supported.
15. OBJECTION CODIFICATION -- In cases where one side does not accept an edit from the other, the objection is formalized as objection or a qualifying adjudication.

=== FEEDBACK PERIOD ===
16. COMMUNITY FEEDBACK -- The larger community is then invited to comment on all parts of the analysis.  The consensus points made, the left and right framings of the issue, as well as all objecting adjudications made by the initial collaborators.
17. RECURSIVE FEEDBACK -- Depending on the resources allocated to this topic, and the amount of contention found a certain amount of recursive adjudication of community adjudications is also performed.
18. SEALED HOLDBACK -- 20% of the adjudications are randomly sealed.  The results of the adjudication vote is counted in as part of the feedback process, but the name of the adjudicator and the text of their analysis is sealed by the system.
19. POSITION EVOLUTION -- Over time the system maintains a current "best" leading edits of the original position, as well as a best list of supports and objections for this best piece.  (NOTICE: only those on the left are free to suggest edits to the left position, and to suggest edits to objections to the right, etc.)
20. CONSENSUS SUMMARY -- After sufficient multi-round feedback and quiecence is reached, the final consensus statement, left and right positions as well as best objections are captured as a completed analysis for the topic in question.  This is published as an completed analysis by the forum.
21. EVOLUTION -- Over time as interest indicates, or as additional facts become available participants may continue to augment or shape the resulting consensus.  Thus important issues may remain "live" for an extended time.  Others will simply stand as an analysis done at a specfic point in time.
22. EDITS -- During feedback each adjudication is added with visibility into all over adjudications and comments made prior to its submission.  Judgements about this adjudication are based upon the precise state of all relevant adjudications at the moment of submission.  But later as new adjudications are added and new discussions are had, each participant is free to EDIT their adjudication's body text, as well as the adjudication's vote as well.  The analysis of each adjudication is performed with the benefit of all published info at the time of analysis.  Thus it is usually beneficial to edit ones adjudication if new ideas become known.  There is a modest impartiality penalty paid when one changes ones own mind, but there is a much larger penalty paid when adjudications are finalized against your position, thus changing that position of one is wrong is generally better.

23. HOLDBACK ANALYSIS -- At a later time these sealed holdback adjudications are randomly sent to multiple participants with similar left-right bias for evaluation.  This is done to assess the assessors.  The goal is to perform a kind of auditing of the specific adjudication that was submitted.  Was it clear, did it accurately track the published rubrick for the conclusions it drew, etc.  The goal is to gather feedback on this adjudicator, as well as a test of the one the adjudication is sent to.  This test check if the hold back recipient is consistent with all other recipients of the same adjudication, and it is a test of the underlying participants original adjudication.
