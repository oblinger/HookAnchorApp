# Housing --
     ...
     <<>>
    <<>> 
    VirtualRent = House*AggRate<<>>12 + Equity*OppCost<<>>12 + FixedFees 
        AggRate = MgtRate*IncRetain - VarFees - Appreciation 
        OppCost = MktRate*GainRetain - MgtRate*IncRetain 
     
    FixedFees = ClosingCosts + CoFee + HouseIns 
      VarFees = ResaleCost + Maintainace + PMI 
     
     
    VARIABLES   (All Rate variables are rate per month) 
     
      VirtualRent   After all "+" and "-" added up this is monthly cost of home. 
      AggRate       Aggregate costs.  Cost as percent of total house value. 
      OppCost       Opportunity cost.  Cost of tieing up equity in home. 
     
      House         Total cost of home. 
      Equity        Total equity invested in home. 
      Appreciation  Appreciation of home's value. 
      MgtRate       House morgage rate. 
      MktRate       Expected rate of return on non-house investments (stock market) 
     
      FixedFees     Fixed costs (that are not a percent of house value) 
      VarFees       Variable costs (that are a percent of house value) 
     
      GainsRetain   =72%.  Amount of money retained after Captial Gains (100%-28%) 
      IncRetain     ~67%.  Money retained after federal and state income taxes. 
    <<>> 
     
    VirtualRent   1273 = 420 + 53 + 800 = 300,000*.00140 + 60,000*.000891 
    AggRate       1.69% = 4.69% - 0 - 3% 
    OppCost       1.07% = 5.76% - 4.69% 
    ...
    Suppose Housing prices were flat:              
    Appreciation=0%, So AggRate=4.69%, So House*Agg=1173 
    So new VirtualRent = $2026   Additional monthly cost = $753 
     
    Suppose the stock market kicks ass:            
    MktRate=15%, OppCost=6.11%, OppCost*Equity=306 
    So new VirtualRent = $1526   Additional monthly cost = $253  (no biggie) 
     
    Suppose Housing mkt drops by 3% annually:      
    Appreciation=-3%, So AggRate=7.69%, So House*Agg=1923 
    So new VirtualRent = $2776   Additional monthly cost = $1503 (ouch!) 
    ...
    Here are the relevant parameters: 
      ...
      R (Morgage rate)       ~7%        Annual interest on loan 
      P (Property taxes)     ~1.5%      Annual property taxes on full value of home 
      K (Kombined rate)                 Combo of annual rates against house cost H 
      C (Closing Costs)      ~5,000     = Points + Title Ins + ... 
      Y (# of years owned)   5          Total ownership period 
      D (Down Payment)       ~ P x 20%  Initial downpayment on house 
      E (Equity in house)    >D >>mo of $$ tied up in house 
      T (Income retain rate) 67%        Money RETAINED after state & fed taxes 
      ...
      M (Market Return)      ~9.5%      Return on money invested in market 
