...
### People 
    ...
    SIMPLE http://www.pipl.com/   azat mardinov
    LISTS http://websearch.about.com/od/freepeoplesearchsites/p/Free-People-Search-Sites-The-Best-On-The-Web.htm

## .Services

#### SpoofCard -- Spoofs caller id   (also BluffMyCard and StealthCard)
...
### .Scraping Discussion With Ben & Rui
...
- PayByGroup merchant generator
  ...
  - PARAMETERS:
    ...
    - Secure parameters are passed by AJAX call
      ...
      (JSON call provides secure parameters and returns a long random ID used 
       to match up the transaction w. client side post made by the button)


API
- generate button --> HTML/Javascript
  - list of params and default values
AJAX CALLS FROM THEIR SERVER TO PBG processing server
- CreateGP(jsonParams)   ;; this call to our server creates a new GP object with some secure parameters set
                         ;; returns a long random ID for the GP (which they must pass to our button code)
- UpdateGP(jsonParams)   ;; adds/changes params on a GP, jsonParams must include 'gp_id'

AJAX CALLS FROM PBG processing servers to their servers
- OrgSignup(gpId)        ;; URL where button redirects to, in order to signup organizer for a GP
- OrgPay(jsonParams)     ;; Completes a GP.   jsonParams must include 'gp_id'
- OrgRelease(jsonParams) ;; Releases holds/data associated w. a GP.  (jParams must include 'gp_id')

## Web Scraping
- Notes  https://docs.google.com/document/d/1ok1ybT4VTPKsSoWEFraDlZPTI54tzSXTZxckEALT-m8/edit?pli=1
- Law    https://docs.google.com/document/d/1N1zP-fA3d-GHYF_7ygk7mPC7n8HnnOO8ny59--3I7vY/edit
# Design Strategies
## White space
...
## Design Bootcamp
### Design Bootcamp Day-1

PATH -  Empathy <--> Define <--> Ideate <--> Prototype <--> Test

PERSONA
  Person -- Name, what they look like.
  Needs  -- What are their pain & pleasure points?  TRIGGERS?
  Triats -- Demographics, age, sex, location, generalizable attitudes & char
  Goals  -- what do they want to accomlish?  How do they succeed?


ECOSYSTEM MAP
- partners, capabilityes, -> costs 


ASKING QUESTIONS
- Generates stories and inspriation
- Eval and assess
- Test and isolate variables


INTERVIWING TIPS
- Never ask "usually"  NEVER!!!
- If someone says "I think"
- No binary questions, no leadning questions
...
JOURNEY MAP
visualize activty over time (even before/after use of product)


POINT OF VIEW
User (DESCRIPTIVE {adj})   needs  (NEEDS {verb})  
 because     (INSIGHT {compelling})

### Design Bootcamp Day-2 IDEATION
...
### Ordering
- User stories
- pull tech requirements out of the stories
