.[[Midjourney]].
  , ,
  , ,
  , ,
  , [Midjourney Obsidian](spot://midjourneyobsidian),
  DELS: ,[Midjourney Obsidian](spot://midjourneyobsidian),
  , ,
  , [Midjourney Note](spot://midjourneynote), 
  DELS: [Midjourney Note](spot://midjourneynote),[Midjourney Obsidian](spot://midjourneyobsidian),[Midjourney Note](spot://midjourneynote),[Midjourney Note](spot://midjourneynote),
# Top
#### kind: BigAI
#### area: CoreDL
#### stance: 
## Sections
### LAMP
#### pri: 
#### contacts: 
#### cool: 
#### job: 
### Cash
#### cash_trend: spiking
#### cash: 
#### cash_available: N/A
#### funding: Bootstrapped – no VC funding (self-funded and reportedly ~$200M ARR) ￼ ￼
### Coolness
#### cool_why: 
#### cool_mission: 
#### cool_balance: 
#### cool_dan: 
#### mission: 
Midjourney is an independent research lab exploring new mediums of thought and expanding the imaginative powers of the human species ￼.
### Deep Learning Projects (DLP)
#### dlp: 
#### dlproj1: Text-to-Image Generative Model&#8203;:contentReference[oaicite:0]{index=0}
#### dlproj2: 
Model Version Upgrades (v2–v5 algorithms)&#8203;:contentReference[oaicite:1]{index=1}
### Job Postings
#### job_why: 
#### job_bay: —
#### job_remote: Software Engineer (Remote)
### Toxicity
## Misc Fields
#### bam:
#### dlp_:
