...
❯ aI No. I think we should go ahead now. And create a DMG file that is ready for sharing. Let's just go ahead and finish this pathway. So you can walk me through it. I'm
  gonna start off by going to the account management page as you suggested here.

⏺ Let's do it. Here's the step-by-step:

  Step 1: Create App-Specific Password

  On the Apple ID page (https://appleid.apple.com/account/manage):

  1. Sign in with your Apple ID
  2. Look for "Sign-In and Security" section
  3. Click "App-Specific Passwords"
  4. Click the "+" button to generate a new one
  5. Name it something like "HookAnchor Notarization"
  6. Copy the password it generates (format: xxxx-xxxx-xxxx-xxxx)

  Let me know when you have the password copied, and I'll give you the next command to run.
...
- [x] This means typing "PJPP" should now show the PP command (and any other PJ submenu commands containing "PP"), rather than showing an empty list.
...
- [x] once all items in a prefix menu are filtered, the whole prefix menu (and it files menu) should be removed
- [x] hook://anchor_popup URL ability to trigger popup on given input
...
- [x]  HookAnchor Rescan - Verbose Mode
...
- [x] Adding ha to the path and the ff command etc. should all be in a script file that is inside the .app and that should be sourced by the user's init upon install.
...
## __ Task Groups __
#### ANCHOR FILTERING
- Alias to an anchor is recorded as a markdown with contents:  (Alias for [[FOO]])
- 
...
- all other scans are passed to load data either by the rescan command, or at exit of the app we checked to see if any of the named scans are due for a re-execution. Notice, right now we check for re-scanning when the application first starts, but this is wrong. Rescanning takes noticeable time so instead, we should be checking it at the end 
- 
- 
- 
- [ ] when we're gonna dismiss the app instead of doing that we can simply minimize the window and then run the appropriate scanner operation.
