
## = CURRENT FOCUS

### - SAVERS
The Miracle Morning
- **Silence** – Meditation or prayer to start the day calmly.
- **Affirmations** – Positive self-talk to reinforce goals and values.
- **Visualization** – Mentally rehearsing success and desired outcomes.
- **Exercise** – Physical movement to boost energy and health.
- **Reading** – Consuming motivational or educational material.
- **Scribing** – Journaling to clarify thoughts, reflect, and plan.
### - Bucketed Day
 (See [[Daily Schedules]])

Idea: Streamline day into 7 buckets shows below and strive to ensure progress made each day in key buckets: Main (+Chew, Coms) & EVE(Self, Out, Call, Learn, Q2)


2023-09
(see [[BKT]])

SUMMARY 2023-Q3

10h			from 7:30am to 5:30pm
- 2h - DAILY	-  Tidy+Wake(20), Nap(30), Exercise(40), Poop 
- 2h - GAP		-	Lounge, Browse, Eat
- 2h - CHEW 	-  Email, First, Slack, Repeat, Papers
- 2h - COMS	-  Meetings, Zoom, Huddle, Phone
- 2h - MAIN	-  MIT, Prime, Work
  
5h 				from 6p until 11p
- 2h - FAM	-  Dinner, Play, Bedtime
- 3h - EVE		-	Fried, Self, Out, Call, Learn, Q2




2023-04

Daily 7,  Sleep 7:

DAY:		  11	 6:30 - 5:30
- DAILY:	5	: Wake/First, : Eli, : Breakfast, 1 Lunch, : Nap, 1 Exercise, 1 Gap
- CHEW:	2	Todo
- PRIME:	4		

EVENING:  5:	6: - 11
- DAILY:	1 Eli
- FRIED:	2
- FUN:	2:


### - Daily Exercise
- RULE: Engage in 30 min of exercise each weekday and maybe weekends

EXECUTION:  
- Most days days to the treadmill
- If not then
- Pickup Eli by fast walking, if not then
- Do a 7-minute workout, if not then
- Do a vigorous walk after dinner


TYPES:
- Treadmill - 
- Pickup Eli - listen on way to Eli - Push self
- Nordic - 


### - A NEW DAY - RULESET

- BEGIN ANEW - Each day I begin anew, the successes and failings of the day before are past, THIS day, this one will be good.
- DAILY then WEEKLY reviews on most days.  Write a JNL entry with (1) gratitude, (2) what's working & not, (3) any updated plans, (4) occasionally retrospectives.
- ALARM - Set alarm, Cash card controls required waking.
- PLAN - Plan day (After Eli, or In Morning)
- WAKE - Waking activities:  Teeth,Dress,Clean  Meditate,Walk wack
- FAMILY - Break for family when need.  Immediate return.
- FOOD - 20min food break is ok, can read
- !!NO OTHER - No other shit!  Need to get down to work
- !!!30MIN - Target 30-min from wake to work
- !!!90MIN - Drop everything and begin work @ 90min after waking
- WORK - DayPlan and flagged items ONLY
- WORK MIN - Must do at least 30 min.  Ideally 3hrs with.  
- SHORT BREAKS - Frequently I take breaks.  set alarm each time:  Read, Chew
- CHEW - After doing work I switch to chew.

TRY:
- Allowing a work break for an extended chew.  with work expected after.
- Allow reading & email during breaks


### - MIT STACKS

==> 5min-intention-walk -> Login -> First song -> Begin MIT

==> 30min-Admin; Eat/Read 1hr -> MIT #2
	(can 'escape' MIT for admin; *MUST* do 30m MIT before eating)
==> Alarm -> Read In Bed -> Set morning alarm
	(set second alarm if not going to bed immediately)


IDEAS:
- BOOT  w/  WIFI OFF  & F1 fullscreen
- 
#### . FAILURE
- 2022-05-06
	- WAKING IDEA - Drawn off course by tempting idea thinking as I wake.
	- DRAWN BACK - As I started to work I kept getting pulled back to idea.
	- READ E-MAIL/NEWS - Read news first.
	- ON TRACK - Eventually took control.  4-hrs lost (including in bed)
	

### - MLC - MULTI LAYER CAKE

Idea: Manage level of work effort in defined layers

Details:
- MULTIPLE LAYERS OF TASKING
	- **Level 1:       MIT** -- _**Hardest to motivate**_, most important, requiring great focus
	- **Level 2:      CHEW** -- _**Repeated**_, easy, quick tasks, OR 
				**FLOW** Timely, easily motivated, impt, timely tasks
	- **Level 3:       FRIED** -- _**Not timely**_, easy/fun tasks, (maybe impt, focused, longer)
				**REST** -- Most _**rewarding**_, fun, recharging motivating activities
- TARGETS - Maintain target layering; try to hit targets each day

| Date       | TARGETS | NOTES                                                           |
| ---------- | ------- | --------------------------------------------------------------- |
| 2022-04-07 | 2/3/2   | Should be able to increase these, but lets crawl before walking |
| 2022-03-01 | 3/1/3   |                                                                 |


### - 7min Exercise
- Alarm is set for 4:30.  
- Stop & Exercise
- Go get Eli
- Shower when I return

FEATURES
- Need to push myself to be _uncomfortable_ during exercise


### - AD - Anchored Day

Idea: Define specific activities associated with three daily hooks

WAKING ANCHOR
- Dress: Bed/Teeth/Scale/Clothes, ?Meditate?
- Desk: Pills, Coffee, Apples
- MIT: 


EAT ANCHOR -- Immediately after lunch
- Plan post-lunch engage period & rest of day


EVENING ANCHOR -- Immediately after Eli is in bed.
- ++ Plan next day; add MIT *completion* plus ~2 other tasks to todo
- Scan emails
- Scan todo list


Waking Rules:
- <30min ready; 30+ MIT; Select Engage Time
- ++ Starts at Engage time.  Plan rest of work day.  Execute.

## = PRIOR FOCUS
### - Block Scheduled Day - July 2021

<9	Uniform, Wake/Meditate/Walk
9	BLOCK #1 - MIT
10:	Breakfast/Emails/QuickAdmin
12	BLOCK #2 - JOB NEXT
1:	Rest/News
2	BLOCK #3 - A LA CARTE
3:	Exercise/Shower
4:	ADMIN
5:	Eat
6	Eli & Uniform
9	Improving / Uniform
11	BED



SUMMARY:
Lets give it a shot.  Many say having a schedule makes a big difference in the day, so lets try it.  The biggest idea is to simply have an alarm set


_
### - Atomic Morning

CORE IDEA:  Regular wake, Spring loaded start, afternoon exercise each day

ROUTINE
- RECHARGE:  Recharge watch right after focus time
- TRIGGER: 	Time for bed alarm
- TIMER:		IMMEDIATELY SET 30 min timer on watch
- DAY:		Update day line for next day
- PLACE:		Ready sox, underwear, shirt, shoes, coat for next day
- BED:		Be in bed before 30 min is up.
- ALARM:		Set morning alarm
- READ:		Read book until sleepy

- WAKE:		When alarm sounds, set snooze, remove mask.
- UP:		Before snooze ends, get up, and cancel snooze.
- PEE:		Go pee taking clothes into bath
- SCALE:		Weigh self, change clothes
- PRAY:		Meditate / pray
- DOOR:		Put on shoes, sox, pants, coat
- DAY:		Look at day-line to organize thoughts
- WALK:		Take walk

PLACEMENT:
- Sox, Shoes, Pants 	by stairs or bedroom door (exiting bathroom)
- Underwear, shirt	near bed (entering bathroom)


BACKGROUND:
- Only get 4 hours per day of focused work.  Need to protect it.
- Using Atomic Habits idea to generate this time.
### --- Three Hours IN Morn
- MIT set evening before
- Sleep before midnight
- Set Alarm
- Quick walk
- No email before 90 min in morning
- 30 min email pause
### --- TTSTTS - Things to Start / Things to Stop ---

- List top start / stops
- Make goal & approach
- Track progress

Strategies: Bundles; Commitments & Devices; Attitude
### --- DAYS OF THE WEEK ---
MON - AF DAY
TUE - UF DAY
WED - BAT DAY
THR - SCRUB DAY		(Only if list gets too long)
FRI - Q2 Morning; Weekly Repeat afternoon; Scrub Afternoon
SAT - UF Day
SUN - Family Day

MTW - Afternoon Chew. (or Thursday morning focused chew)


2021-01-11 - Created
_


## = BEST HABITS
### - THE ENVELOPE - Brutally effective self accountability ---
#### THE METHOD

CURRENT DETAILS:
- ACTIVATION LOCATION: on night stand after meditation.  OFF = on floor
- SPACE CADET EXCEPTION -- you just forgot the rule.  BUT for this exception to apply as soon as you remember, you need to execute IN THAT MOMENT.
- WRONG CHOICE EXCEPTION -- anytime following the rule would clearly be very wrong or not the intent of the rule, then it is excluded.


SUMMARY:
- WRITTEN RULES -- summary on sticky note.  detail can be listed below here.
- LOCATION -- location of letter determines if the the rules are active or not
- EXCEPTIONS -- detailed list of exceptions or mods make trigger precise
- NEVER FLINCH -- *always* flush the cash on failure, then update the exception cases with an appropriately broad exception case for next time if needed.



KEY EMOTIONAL LESSON:  We are programming our brain to emotionally accept one simple fact.  Validity of the excuse is irrelevant.  If the rule is broken the cash with flush.  >>>PERIOD<<<. Yes this does mean someday you will consciously need to choose to flush the cash even when it was the correct choice to break the rules.  True.  But that cost is so so so much less than the cost of not following ones truest intentions.  But does highlight the need to list many exception cases a head of time, and it highlights the need to not choose a cash amount so big it cannot be flushed even in a case when breaking the rule was correct.


LETTER
I wrote this letter on DDD and promised I would give it away the day I failed a commitment to myself.  Well today I BLEW IT!  My loss is your gain!  Please use this cash to do something nice for others or yourself.  
Cheers!


STORY FORM
I have used this for years, it is a brutally effective way for you to become your own accountability partner.  It flipping WORKS!!!  But be careful it is easy to be too brutal on yourself!


Sometimes our impulses of the moment, and self excuses, override our true intentions.  This commitment device is nearly guaranteed to fix this mismatch between what you say you want in the cool light of day, and what you do in the heat of battle.  If you follow it, you will either change your mind about what you really want, or you will change your behavior...  if you follow it, there is no other option!  (Careful what you wish for, its BRUTAL!)




Here is a brutal but very effective commitment device:

-1- Put an amount of cash in an envelope that is personally noticeable, but also not so large that you couldn't afford to loose it a couple of time in a year.
-2- write a short note. My loss is your gain, use this cash for some kind of good, for someone (perhaps even yourself.) Seal the envelope.
-3- On a sticky note on the outside, or somewhere safe write down your commitment. e.g. I will exercise first thing on all weekdays unless I have a morning meeting. think hard to figure how to exclude all legitimate exception cases.
-4- Place the envelop in a specific visible place. that means it is active. (you can 'pause' it by putting it in a drawer the night before.)

Now any day that you fail to execute, you take the envelop and drop it outside somewhere. KEY: You really need to actually DO it. After dropping it once or twice it becomes a MASSIVE motivator. (but it is important to have a way to 'pause'.... e.g. you are very sick, or you were up all night.) I find that I rarely pause for dumb reasons, and it really does push oneself.

You can update the sticky note as your goals shift.


~-~~
#### 2023-01-09 Wake-First-MIT

- TRIGGER - When first sitting down in front of the computer
	- RULE:  Triage day in first half hour.  
		- Generally triage email at same time, pushing social and medium stuff to in2, and bigger tasks onto todo
	- RULE:  Do at least 1/2 of productive work.   (Generally on a MIT tasks)
	- RULE:  During this time:  No reading medium, and no free cell.

ISSUE: Stress it causing me to fritter time, big chunks are lost at days start.  Partial fix is to get on track first thing.


#### 2022-01-27 - 30/30/30 - Morning Effiency & Focus on tough task
- HARD READY: after weekday meditation&bath set 30 min timer. 
		  then start MIT immediately after (within 30 minutes)
- HARD MIT: set 30 min timer & work in focused way; reset timer if distracted.
- OPTIONAL: (if hungry)   eat + read email&firefox for HARD 30 min before MIT
- SOFT: Work for 1.5-3hrs
- SOFT: After dinner: set MIT & tasks for next day, clear desk and eamils.
- SOFT: 7-min exercise in late afternoon & walk to pickup Eli too.
- MEDITATION EXCEPTION: if something thwarts meditation, then rules are soft.
- URGENCY EXCEPTION: No email/web before MIT unless I have reason to need to.
- HUNGER EXCEPTION: If hungrier than 2apples, do 30min eat & read.

#### 2022-01-27 - 30&30 - Morning Effiency & Focus on tough task
- HARD READY: after weekday meditation&bath set 30 min timer. 
		  then start MIT immediately after (within minutes)
- HARD MIT: set 30 min timer & work in focused way; reset timer if distracted.
- SOFT: Work for 1.5-3hrs.  7-min workout at end, then eat, then admin.
- SOFT: After dinner: set MIT & tasks for next day, clear desk and eamils.
- MEDITATION: if something thwarts meditation, then all rules are soft.
- URGENCY EXCEPTION: No email/web before MIT unless I have reason to need to.
- STARVING EXCEPTION: If hungrier than 2apples, do 30min eat & read.

### - PDP - Procrastination Defense Protocol

**HARD RULES**
- 30 min eat & email ???
- 30 min uninterrupted work


**RULES**
- **ALARM** -- Set alarm for 7 or 7:30 hours of sleep.  
	(can cancel/update anytime as needed)
- **ANCHOR** -- It is critical to execute the morning anchor each weekday.
	- **NO EMAIL** -- Don't read email or RSS until complete
	- **REAL 30 MIN** -- Don't count time that is not really executing well
	- **THOUGHTS** -- Can only transcribe thoughts into bullets into scratch
	- **EXCEPTIONS** -- Waking after 10; External interruption; scheduled task
- **PENALTY** -- Must drop cash card that day for ANY violation of anchor rules.



-1- Put an amount of cash in an envelope that is personally noticable, but also not so large that you couldn't afford to loose it a couple of time in a year.




-2- write a short note.  My loss is your gain, use this cash for some kind of good, for someone (perhaps even yourself.)  Seal the envelope.




-3- On a sticky note on the outside, or somewhere safe write down your commitment.  e.g. I will exercise first thing on all weekdays unless I have a morning meeting.  think hard to figure how to exclude all legitimate exception cases.




-4- Place the envelop in a specific visible place.  that means it is active.  (you can 'pause' it by putting it in a drawer the night before.)







Now any day that you fail to execute, you take the envelop and drop it outside somewhere.  KEY:  You really need to actually DO it.  After dropping it once or twice it becomes a MASSIVE motivator.  (but it is important to have a way to 'pause'.... e.g. you are very sick, or you were up all night.)  I find that I rarely pause for dumb reasons, and it really does push oneself.




You can update the sticky note as your goals shift.


_


## = OLDER
### - HEALTHY DAY
WAKE
- FRUIT -- Cut up fruit basket
- VITAMINS -- Take your vitamins
- FLACKS -- Get out flackseeds
- FRIDGE -- Ensure foods in fridge (from freezer)

EVENING
- TEA -- Make teapot; Add tumeric & ginger spices (?soy ?honey)
- YEAST -- Eat a bit of yeast

_
### - Loosely Scheduled Day - June 2021


06a	Wake
07a	Slot #1 -- Core work
10a	Food
11a	Slot #2 -- Core work
01p	Admin
03p	Exercise
04p	Slice #3 -- Secondary projects (only if 3hrs of core)
09p	Slot #4  -- Ranging activites


IDEA:
-- I rapidly move towards this loosely structured day
-- It gets efficiency by maximizing effective hours


_
### --- SCHEDULE: Eli centered Days ---

 5:00	Ready
 5:30	ME block 	(2h)
 7:30	Eli 2 school	(F1)
 8:30	Breakfast
 9:30	Slot1 		(2h)
11:30	Lunch
12:30	Slot 2 		(1h)
 1:30	Nap
 2:00	Admin
 3:30	Exercise
 4:30	Shower
 5:00	Eli				(F4)
 5:30	Cook
 6:00	Eat
 6:30	Clean
 7:00	Eli - play
 8:30	Story Time
 9:00	Evening Relax
10:45	Sleep
_
### --- BIGGEST BUCKETS ---

- SLEEP	7:3	--	Sleep, Nap
- DAILY	3	--	Wake, Eat, Physical
- FRIED	3:3	--	Fried, Browse, Read, Gap
		  14
- CHEW	2	--	Chew
- PRIME	5:3	--	MIT, AF, Q2, UF
- HUMAN	3:3	--	Family, Spiritual
		  10


					D	C	F	H	P	S
				+--	--	--	--	--	--	-- -+
 5:00	Wake	|	1			/			|
 6:30	MIT		|			/		3		|
10:00	Bfast	|	1						|
11:00	First	|		1/					|
12:30	GAP		|			1				|
 1:30	After	|					1/		|
 3:00	Human	|			/	1		/	|
 5:00	Dinner	|	1						|
 6:00	Eli		|		/	/	1			|
 8:00	Evening	|			1	/	1		|
10:30	Sleep	|						6/	|
				+--	-- 	--	--	--	-- 	-- -+
					D3	C2	F3/	H3	P5/	S7	= 24
_
### --- DAILY SLOTS ---

IDEA -- Plan out discretionary slots in day


**SCHEDULE**
5 	Wake
6:			Slot #1 -- MIT
8	First
8:			Slot #2 -- MIT
10	Gap
10:	Chew
12 	Lunch
1 	Gap
2			Slot #3 -- Afternoon
3: 	Nap
4			Slot #4 -- Quick Dash
5 	Dinner
6 	Evening
7 	Eli
8			Slot #5 -- Evening
9:	Gap			
10:	Sleep


_
### --- 5AM WAKE ---
### --- Daily Orientation ---
- walk outside for a short bit of orientation on the day.
- be sure to think about how well you are doing on habits and goals.
_
### --- DTOT - Daily Time On Task ---

5	Wake
5:	Fun Orient
[6	SLOT-1M	-- MIT
7:	CHEW 	-- FIRST/CHEW/BFAST
8:	-slack-
[9	SLOT-2M -- MIT
10:	-slack-
[11	SLOT-3F -- Fun Slot
12:	LUNCH
1:	-slack-
[2	SLOT-4S -- (tier II or run)
3:	CHEW	-- CHEW / FUN TASK
5	-nap-	-- somewhere-before-this--
5:	Dinner
6:	Eli
[7	SLOT-5F -- UF
8:	SLOT-6F -- UF
10	BED

_
_

### --- Path Visualization ---
-- Visualize correct actions:  Traction/Distraction
### --- The NOT todo list 

- Skip morning MIT
- Spend >30m eating  
_
### - HOOK HONING

Define:
- Hook name and letter prefix
- Rules for correct execution
- Metrics for success/failure/daily tracking

Idea:
Identify a hook, the rules, and metrics around the hook.

Idea: Have a set of strategies and triggers that best leverage my motivation and energy.

_
### - HOOK-HONING - Daily Triggers
Hooks:  W,M,L,A,F

**WAKE**:  	<5 resleep.  <30m get up.  >6 quickwake.  7 Alarm 
		W:  <1h up and <1.5h working
**MORN**:	Dig in for 2-3 hours
		M:  MIT >1.5h before 2p 
**LUNCH**:	After Lunch:  Sales30   Grid/Log/Chew
		L:  Lunch <1h.  Sales >10m.  ChewFirst
**AFTER**:	<1h  Re-engage:  2-3hr Block2&3 Task
		A:  Plan-and-do.  >1.5hr
**FRIED**:	Decide.  BeFocused.  Fried/UF/Read/Fam/Browse.
		F:  Make good use of fried time, however its done.
		
_
### - HDA - high density action

Key idea: Do M/S/C and Recharge

Rapid Recharger -- Nap, ?browse?, Exercise

MUST	--  Do or die tasks
SHOULD	--	Avoidance prone, important tasks
COULD	--	Phillic, valuable tasks
WILL	--  Automatic background tasks, that I will do w/o issue
REST	--	
QUIT    --  

_
### - SALES FIRST

APPROACH
- FIRST -- Create a section called "first" in my todo.
- AT START -- Attack items in this section "first" even before chew first. At least 30min spent. 
- ???? -- regular times to do sales planning?

Background:
- I hate the tracking of sales.  hate it.
- But I am not being insta-responsive in a way that I must.
_
### === OLDER STUFF ===
### EFFICIENT DAY -- 2019-10-15 - thinking about WHY -- 

RULES
- BED -- In bed by 11:00p -- maybe reading a bit
- ALARM -- 7am -- 


Recursive Problem Statement
- I want to have an unbreakable and efficient rhythm.  Why?
- I feel good when productive & want to accomplish before death.  
- Is exiliarating to do.  feel bad when day acheives little.
	ego is tied to it.  my contribution.
### Quick Healthy
- WALK ON ANY CALL OR THINKING
- G-MA FOODS - No eating at night
- 5-min workout each day?  (4 day/wk)
### BIG ROCKS
Related to two chunks.
Idea: Need to move the needle on the big chunks.  Rest will take care of itself.

Right Now There are Two:
- SALES PLAN
- UF TODO
### TWO CHUNKS
Idea: Try to get two 2-3 hr chunks of focused work done each day.
- ideal is to work continuous for 2-3 hours.  but still counts if total is >=3 hrs
- ideal is one of my defined projects, but can be any work (even running errands)
- The expectation is that each day will also have other things taking time away from these.
_
### Ideas
-- Contrast / Affirmations / Breathing / Quotes / Journal / 1Win / 
-- 7min&Stretch????

-- Read one (big menu)
-- Chew First & Urgent
-- MIT
-- 1Win
-- Celebrate

-- Schedule
-- 50min Admin
-- 100min task
### === LONG TERM CHANGES ===
[Long Term Change](https://medium.com/better-humans/the-ultimate-model-for-successful-and-long-lasting-behavior-change-221e678c642b) 
### === OLDER ===
### MOTIVATION
rewards, consequences, and commitment devices
https://medium.com/better-humans/the-best-last-motivation-article-youll-ever-read-4-tactical-tips-to-turn-self-motivation-into-2337259472ed
### UNCOMFORTABLE
- cold in the morning
- 1-min exercise
- read below
http://www.willpowered.co/learn/strengthen-your-willpower
https://medium.com/@ngoeke/how-to-increase-your-willpower-822fd7a4a3ff
### 2-2-MIT
Begin a MIT task of at least 30 min duration, no more than one hour from the moment of waking.
### Morning Groove
2-2-MIT
PrePlanning -- of the day; of the week; of the quarter
Gratitude -- 
### IF-day
- Aim for a 8hr feeding window:  noon-8
- Coffee/Tea; but no calories;  Occasional diet drink
- Last meal: fiber, fat, protein
- Break fast with bone broth
- Consume carbs seperately from fats
- Exercise during fasting period

- LUNCH:  WetStarch, Veggies
- PREDINNER:  
- DINNER: Fiber wafer, Meat, 
### ASMR sleep
https://www.yahoo.com/lifestyle/11-asmr-videos-sure-best-183641043.html?guccounter=1&guce_referrer=aHR0cHM6Ly93d3cuZ29vZ2xlLmNvbS8&guce_referrer_sig=AQAAAFNaM82rLezOSNdJ_qJNDM0UFzVvRGosbrWSzJiPl9838cHjvyA9kXcNgpIAGe_Ag8ITQ-DByIKw5ndLJG8kn7R16rUWACJdYRXXH61wrU0NCdlQ2P1vjQapJDHguJB-X8l5rcQbRE-ek3pPMmsdYMRBmlDnTUR_i95Ar0mq2frP
### Potcasts
### Traunched Work

MINIMUMS
MIT: .5/day.  3hrs x 2Week = 7.5/week
Q2:  

Idea:
We break up our work into tranches, then set daily/weekly targets by Tranche

GROUPINGS
	MIT = MIT, AF MIT
	Q2  = Q2-plan, Q2-MIT,
	1st = Chew
	2nd = Daily


Definitions:
- MIT    work -- Specifically designated most important tasks
- Q2     work -- Strategic work
- Prime  work -- Clearly work, needs to get done regularly
- Second work -- Good
- x
- x
### Exercise ???
### Self Reflections ???
### Toughest first
### LOW CARB & EXERCISE
### WEEKLY TARGETS
10%	5	First
14% 7	Chew
10%	5	MIT
 6% 3	Exercise
20% 10  AF
40% 20	Uniform
    --
    50  = 5 x 10h

	60  = 50 + 10  Totals working hours per week


Daily Rhythm
7a wake
8a chew    11-3h = 8h working
7p dinner
9p evening slot = 2h working
11p sleep
### MIT TRIGGER
~ Spend >10min at beginning of each day on MIT.  
~ Must start after initial planning/email <1.5 hours.
~ If day starts in flow then MIT <6hours.
### FIRST THINGS
ASANA
-- Click Thru Asana tabs skipping Todo
-- Glance at HABITS
-- Finsh on Todos and figure out MIT & goals for the day
EMAIL
-- Triage email

Background:
-- Aim to trigger impt activities in a seamless flow chain that begins each day
-- Daily Check-in
	* Just look at MIT & work on for 2 min each day
	* Look at habits list daily too
### Mission Orientation
### === Other stuff
### HOURS

Hours Available:
 1  6:00 UP
"   7:00 Email
3	7:30 Block-1
 " 11:30 Breakfast
3  12:00 Block-2
 "  3:00 Nap
1	3:30 Admin
 "  4:30 Cook
 1  5:00 Eat
2   6:00 Block-3
 1  8:00 QL
2   9:00 Block-4
 7 11:00 BED 

Best Case:
10h	  working (Good)
11"h  working (Aggressive)
7"h	  sleep/nap
5h    other

#### HACK DAY

Hours
  "	 6:00 UP
"	 6:30 email/quick
4	 7:00 Block-1
  "	11:00 breakfast
3	11:30 Block-2
  1	 2:30 Lunch/Nap
3	 3:30 Block-3
  1	 6:30 Dinner
3    7:30 Block-3
  "	10:30 Ready
  7.11:00 BED

13 Hack
 4 Other
 7 Sleep
### Articles
GRIT habit -- https://medium.com/the-mission/willpower-doesnt-work-here-s-the-key-to-being-more-productive-according-to-neuroscience-f6c4257eb6b9 
### === OLDER HABITS ===
### HEATHY MEALS
-- make a few healthy meals.
-- mess around with meal timings and amounts to manipulate hunger (get scale out)
## = HABIT THINKING
### --- HIGH DENSITY ACTION

**GOAL:**  Spend my time (1) effectively and (2) in right areas

**ISSUE:**  Sometimes I cant force myself to make effective progress.
- At certain times certain tasks seem DRAMATICALLY harder for me to force myself to make progress on.
- Even when I managed to force myself to stay on task, I often don't make good progress.

**EFFECTIVENESS MEASURE  ==  task_importance  *  %task_effectiveness**
- The idea here is that we measure effeciveness as a combination of working on the right task, and our effectiveness on that task relative to some mytical 100% effectiveness rate.
- 100% effective means the speed and quality that Dan would achieve on his best day in the best mind, etc.

**IDEA:**
- SWITCH -- when I am not making good progress, I am very aware of it.  So just SWITCH!  Allow myself to freely swap off towards any high-ish priority task for as long as I want.
- SWITCH BACKS -- if I find that a high priority activity is languishing, keep restarting that activity, and keep allowing myself to switch away.
- LOOKING FOR THE CATCH -- I notice that once an task "catches" for me, I tend to hyper-focus, so I am just giving myself many opportunities to 'catch' and hyper focus on each high priority activity naturally without force.
- BACK STOPPING -- but this process could result in endless slipping on important but distastful tasks.   To avoid this we need to tracking when important task need to be tackled, and be sure we are eventaully disabling the "switch away" option until they are done.

**MAKING IT WORK:**
- ACCURATE TASK QUEUES -- Need to maintain task queues with accurate realtive prioritization and accurate due dates.
- HOT SWAP COINTEXT -- While tasks are in the "hot swap" list the context for action on those tasks need to be current and at "top of note"
	- Each 'hot swap' activity should be indexed by a spotlight_commander term.
	- The target of that term should be a Folding Text page.
	- That FT page should open to a section where this hot-tasks status is shown
	- Status info -- first line:
		summary of current goal
- PRIORITIZATION -- [msc]
	Prioritization of tasks should be organized according to consequences of action (or non-action).  One can should also order tasks within each category according to priority.  But these categories are important water marks since they indicate different stances toward a task:
	- MUST -- Tasks that 'must' be done otherwise there are unacceptable consequences.  Typically tasks don't get bumped up into the 'must' cateogory until they become critical.  e.g. a task that "must" be done, but could get done next week, does not count as a MUST task.  It only gets elevanted to must when further delay is unacceptable.
		The idea is to keep this category very very empty, so when something *IS* in this category and I am not doing it.  It is a screaming red alert -- no hiding that I am SCREWING UP ROYALLY.  It should never be normal to have things in the must category, and I am doing a non-'must' task.
	- SHOULD -- Tasks that are often pushed aside end up in this category.  they need to be push above others so they get attention.  They are things you "should" do before they become tasks you "must" do.
	- COULD -- Tasks that I "could" do at most any time and it would be a good thing to do generally.  These include the daily tasks like reading email, or showering.  They are not in the "SHOULD" category since no special pressure is needed.  This system is designed with the idea that there is time to accomplish all the "could tasks" and one naturally will do them all.  So category is not really involved in the pressure dynamics, these are just in the background. 
	- MIGHT -- Tasks that are important but are also kinda fun and so they can eclipse more pressing action.  These tasks are often the things I do when I "should" or "must" be doing something else.  These tasks are important, and they do deserve my time.  But they can sometimes be done so much that I starve out more important or more time critical tasks. 
	- SHOULDN'T -- These are tasks that have value or enjoyment.  Still given the finite nature of life, these are ones that I have decided I want to minimize.
	- MUSTN't -- These are activities that are toxic enough and with so little redeeming value, that I have decided I would be better off NEVER doing them.  (of course the spirit is willing while the body is weak, so we have them listed and tracked.)
	
- FLAGGING -- A way to measure and coax good behavior is to assess fractions of time spent in the wrong category.  Here are the measure of performance:
	- EFFECTIVENESS [0 - 100%] -- This is an overall "gut" assessement of how well you were doing what you were trying to do, compared with the quality and velocity relative to your "perfect day".  So 50% would mean taking 2 hours to do a 1 hour task, but achieving the same quality.  Difficult formalize how to penalize for drops in quality.  Really depends on the importance of the quality relative to outcome.
	- MEASURING EXECUTION FAILURES -- measured by day, and are measure in both in hours and in number of items. There are four flag types:
		- MUST-RATIO --  Your GET-ER-DONE ratio
			Number and mount of "must do" items that were completed in time.  not worked on enough to complete within allotted time.  Time measurement is amount of additional time that would have been required, which was spent on non-must tasks.  The cards are down, all your chances to get this done in advance are gone.  It is clutch "do or fail time".  The target here is 100% m
		- MUSTN'T-RATIO -- Your BACKBONE ratio.
			Number and amount of "mustn't do" activities done.
			You just said you were never gonna do that again, but there you did it again.  Yo!  Grow a spin
		- SHOULD-RATIO -- The BACKBONE ratio
				Ratio: must+should / all_except_could
				This is a measure of how well I am allocating my discretionary time.  Notice we have remove the "could" category.  No credit or penalty is assessed if you have a day with many doctor's appointments or emails to do.  you only get a low score if you should have been doing that sales deck but instead you put all your time into your side project.  Notice the goal is not 100%.  For me I would be happy with even a 40% level since it force strong motion on tasks I "should" be doing.
		- COULD-RATIO -- The ADMIN raio
		


### >>> LONG TERM HABITS <<<
