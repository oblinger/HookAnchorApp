


- Roth Conversion

- 1 password

- EVERNOTE - consider switch to evernote
- INFO SEARCH: [F]inder.  [S]earch.   [G]rep.
- SCRIPT: Add URL from finder into folding text
- Mint spreadsheet

- Heart health institute
Nick meeting days (pick them with him)
401K move to ROTH
fix archiving of email
allison 'wish list'
yashin; watch faces
glucose monitor (buy it)
Jared:  Facial counter strain, glocose Libre (Zoe projec
Wider laptop desk
quick mint budget
research multi-vitamin and omega 3 to buy
merge photos (read about)
buy photo frame

- Dogtra; ecolar 
[kubernettes](https://lzone.de/cheat-sheet/Helm%20Templates) 
 [IF](https://jamanetwork.com/journals/jamainternalmedicine/fullarticle/2771095?guestAccessKey=444bbcb2-7e13-4dc6-998f-5de5e27aa19e&amp;utm_source=For_The_Media&amp;utm_medium=referral&amp;utm_campaign=ftm_links&amp;utm_content=tfl&amp;utm_term=092820) 
- figure out new meals
- check daring fireball's markdown to html
- make scan generage aXXXX for all .md files

- clean basement
- fix sleeping stuff
- photos on computer
- NSTR load
- BabyRoom Lighting (wall light)
- clean up _notes.md

HACK — Choose Tensor flow / pytorch / ???
— Find tutorial on Tensor Flow Keras
— Build environment.


LATER
- [blinds](https://www.theverge.com/2019/1/9/18175640/ikea-smart-blinds-blackout-us-store-availability) 

### --- Heart Institute

- CPMC - Heart Disease Reversal Nutrition Program: CPMC 415-353-4278
- Stanford Preventive Cardiology -  (650) 725-5909

### MINT
[Groc](https://mint.intuit.com/transaction.event#location:%7B%22accountId%22%3A7132218%2C%22query%22%3A%22description%3AAUTOMATIC%20PAYMENT%20-%20THANK%22%2C%22offset%22%3A0%2C%22typeSort%22%3A8%7D) [Amazon](https://mint.intuit.com/transaction.event#location:%7B%22accountId%22%3A6984777%2C%22query%22%3A%22description%3AAUTOMATIC%20PAYMENT%20-%20THANK%22%2C%22offset%22%3A0%2C%22typeSort%22%3A8%7D) 
[Sapphire](https://mint.intuit.com/transaction.event#location:%7B%22accountId%22%3A7132216%2C%22query%22%3A%22description%3AAUTOMATIC%20PAYMENT%20-%20THANK%22%2C%22offset%22%3A0%2C%22typeSort%22%3A8%7D) 

# TOP
## === SOON ===
- one button night noise / Biurnal beats / Use shortcut
- 
_
## === QUICKIES ===
- hand sweeper to basement & sweep car
- spot scan: sees 'afsales' with NICK AND DAN and Nick And Dan


## === FRIED FLOW ===
- CLEAN: 	In2 * House * Basement * Scratch * ToRead
- WEEKLY: 	FILE THIS * TO_BUY * TIDY * EMAIL JOY *
- QUARTERLY:	Mint * 
- CALLS		Mom&Dad * Repeat * Habits

- look at habits
- look at planning
- https://futureofcoding.org/community

_
## === QUICKIES ===
- Clean basement
- Fix chair feet in dining room
- Estimate Water Usage
- get list of hikes figured out
- Fix clock
- Org these lists
- Reset door codes. (info in myfo->mystuff)
- Use WhatSize to clean disk
- Open Amazon card
- take notes on using the call recorder
- FOOTPRINTS - pick photos, print photos, build, wrap
- [Udemy - Tara - Sleep](https://www.udemy.com/heal-anxiety-sleep-easily-tara-brach/learn/lecture/14770060#overview) 
- Russell Keith Magee  Transpile Python
	https://www.youtube.com/watch?v=9c4DEYIXYCM
- reset mac for headset bluetooth connect
## === EASY ===
### --- clean up to read folder ---
### --- Emails backup
- Verify which emails and drafts are being saved.
- which are missing?  from which accounts
hard core backup of all emails . (all funnle in, then all get recorded)
### --- Wedding Album
Create Small album of wedding photos
-- Clean up car
## === THINK ===
### hack add .md support for spotlight  (think sematnics first)
able to easily add spotlight links for .md files
should not need 'about xxx.org' 

write up good semantics!!
_
### PHOTOS
- buy big viewer
- figure out if using google to share is good idea
- copy all QL photos into Dan's apple stream?  then pull 'bests' out of stream?
THINK!  write goals, and options


Learn how apple photos works
_
### CRM 4 SELF
personal CRM to track my own contacts?
# LISTS
## === [TO_BUY](~/ob/topics/buy/tobuy/__tobuy__.md) ===
-- oximetry
-- circuit breakers for house
-- buy dirt
_
### --- NEEDS RESEARCH ---
-- Some kind of blocking screen for bedroom
-- black out blinds - electric blinds
-- headset that does not require power-cycle
-- Shoe rack for hallway
-- [digital scale](https://www.amazon.com/Etekcity-ESN00-Nutrition-Counting-Bluetooth/dp/B07FCZSC41/ref=sr_1_4?dchild=1&keywords=wifi+kitchen+scale&qid=1595817273&s=home-garden&sr=1-4) 
-- safe for valuables
-- cameras for in house / surveillance for outside house


- weighted blanket
-- bedroom alarm clock
-- Survellance camera
-- IOT light switches
-- drawers for office
Living room cabinets (paperwork in cave)
- Bedsheets matress cover for closet bed



_
### --- BABY BOOKS ---
https://www.the-best-childrens-books.org/8E.html
https://reedsy.com/discovery/blog/best-childrens-books

https://www.npr.org/2020/08/31/905804301/welcome-to-story-hour-100-favorite-books-for-young-readers

## === TO_HACK ===
- #phone. working for evernote
### --- to consider
HACK -- get all mail messages logged

-- Calendar automation
-- Zoom schedule . #Zoom (should bring up notes)

-- move paypal so we never use oblinger@gmail.com

-- strip broken CMD links
-- add media keys to the default touchbar
-- Sublime MD folding
_
### --- ONE LINERS ---
- email drafts:  loosing them; many copies
- dan@analytics email does not get logged!
### --- PHOTOS ---
-- understand how apple photos sync
-- decide on my process
-- maybe update dad's process
-- quick organize of photos 
-- and apple photos too
### --- TRACKER
-- Want to see slices of work to see where to optomize
-- 5-slice model by day by week w. percentages
-- 20-100 slice model by day by week

- Compute m/c/s for each entry and show in report
- 0min gets recorded and hide window
- record entries for prior days
- show reports (just try it)

~~~ OLDER ~~~
- summary groups
- life paper house
--  notice idle time

-- Ability to show defined groupings "1" "2" ...

-- ability to specify activty UPTO certain time
.

-- Double entries are merged when saving
-- Print report w. summary categories added
-- life tidy-->clean .  cooking-->food
-- life-->daily . exercise




-- CMD-P to raise control window

=== GUI ===
GAP time amount
CURRENT TIMER:  time left, time accrued, category
NEW ENTRY: category duration starttime



=== COMMANDS IN WINDOW ===
 
CMD-P -- hides window
      if timer is not running then it starts next default pomodoro

SPACE [ <CAT> ] [ <TIME> ] SPACE comment string RETURN
    Adds a new time entry.  default category is current category.
    Default time is all the gap time

"/"  show categories list (shown a dense block)

-- 
- tracker scan . fails
- remove duplicate cal entries from two calendars
## === TO_READ ===
### _
- Miracolo eucaristico
- Enclical. Human vitae.  Vows.   Love letter.   Spiritual plan
- Nick haskell module that parses many languages.
- Markus Aralius meditations -- have a short bit before I sleep or when I wake of uplifting or though provoking reading
### [Mental Models List](https://medium.com/@yegg/mental-models-i-find-repeatedly-useful-936f1cc405d) 
### outsourcing
Dear Dan,
 
I am sharing here a whitepaper on selecting partner for product development.
This discusses about the various areas such as mutual synergies, expertise, experience, terms, and more.
 
http://www.damcogroup.com/wp-content/uploads/2018/06/Guide_How-to-Select-Right-Partner-for-Outsourced-Product-Development_Damco-Solutions.pdf
 
Hope you find this information useful.
Have a great day
 
Best Regards,
Rajat
 
Rajat Kumar Naik
Vice President, Sales
Damco Solutions, Inc.
2 Research Way, Princeton, NJ 08540.
Ph: 609.246.5966 | Email: rajatn1@damcogroup.com | www.damcogroup.com
https://www.damcogroup.com/product-engineering-services.html
### Nick Rascal
https://www.rascal-mpl.org/


Rascal [integrates] source code analysis, transformation, and generation primitives on the language level. Use it for any kind of metaprogramming task: to construct parsers for programming languages, to analyze and transform source code, or to define new DSLs with full IDE support.
- We are legion we are Bob.  Derrick. 
- is a slow less dense read: [books](https://www.amazon.com/Varieties-Religious-Experience-Penguin-American/dp/0140390340/ref=asc_df_0140390340/?tag=hyprod-20&linkCode=df0&hvadid=312607785736&hvpos=1o1&hvnetw=g&hvrand=17380473608366033599&hvpone=&hvptwo=&hvqmt=&hvdev=c&hvdvcmdl=&hvlocint=&hvlocphy=9061273&hvtargid=pla-414961594307&psc=1&tag=&ref=&adgrpid=61775261186&hvpone=&hvptwo=&hvadid=312607785736&hvpos=1o1&hvnetw=g&hvrand=17380473608366033599&hvqmt=&hvdev=c&hvdvcmdl=&hvlocint=&hvlocphy=9061273&hvtargid=pla-414961594307) 
### --- RE-READ LIST ---
- [coach](https://betterhumans.coach.me/the-ultimate-model-for-successful-and-long-lasting-behavior-change-221e678c642b) 
- [Occasionally Ask](https://medium.com/mind-cafe/8-great-questions-you-should-occasionally-ask-yourself-ebc929094777) 
- [Markus Aurelius 10 rules](https://medium.com/swlh/marcus-aurelius-10-rules-for-life-dfbdc9869e68) 
- 

_
## === TO HOUSE ===
- Anchor closet and shelf to protect Eli
- Put feet on living room chairs
- BASEMENT -- mount shelves so they cannot fall
### --- HOUSE LATER ---
- Raised deck box
- House Remodel
- Fast Water Heater.  866 486-3669 . 
- Shades -- just buy some (call Bali)
- BEDROOM --  Closet Pole
- BASEMENT -- Add light controls
## === LATER ===
 - Figure sit outside solution
 - Create 7-min workout .  @ "7min" in contacts
_
### [[NEVER]]
RESERACH -- high carb to break fast; low carb to start it
- BUY [Sparkling Water](https://www.amazon.com/SodaStream-Source-Sparkling-Water-Starter/dp/B00EPEALGU/ref=cm_cr_arp_d_product_top?ie=UTF8) 
- Buy Apple TV for them (parents)
# ### LOG ###
### --- photo sharing ---
### --- photo frame --- 
### --- Find Foods ---
 - and list of dinner add ons (veggie)
 - LUNCH LIST - build list of lunches (paprika)
### --- High brow courses ---
https://gohighbrow.com/start-the-course-from-email/?email=highbrow@oblinger.us&course=Negotiation%20skills

https://gohighbrow.com/portfolio/mens-guide-to-grooming-and-style/

https://gohighbrow.com/portfolio/drive-sales-with-inbound-lead-generation/


https://gohighbrow.com/portfolio/persuasion-science-masterclass-part-i/
https://gohighbrow.com/portfolio/building-a-network-for-success/

https://gohighbrow.com/portfolio/integrative-thinking-a-practical-guide-for-leaders/
https://gohighbrow.com/portfolio/introduction-to-design-thinking/



https://gohighbrow.com/portfolio/the-ultimate-beginners-guide-to-wine/



https://gohighbrow.com/portfolio/how-to-outsource-your-diet/

https://gohighbrow.com/portfolio/most-brilliant-social-psychology-experiments/
https://gohighbrow.com/portfolio/human-history-and-the-first-civilizations/
https://gohighbrow.com/portfolio/vikings-history-and-mindset/


https://gohighbrow.com/portfolio/the-worlds-most-compelling-logic-puzzles/
https://gohighbrow.com/portfolio/brain-twisting-paradoxes/


https://gohighbrow.com/portfolio/greatest-us-presidents/
### Scott Hoffman video
Here is the link, hope you enjoy it 
https://vimeo.com/291423665

ThanksScott hoffman
### hbr sales

http://hbr.org/1996/09/building-your-companys-vision/ 
### calendly
Dan

Have you seen Calendly?

I do it to let people book meetings with me via a URL. Eg:

http://calendly.com/nickallen/30min

It lets you define ranges that you want people to be able to select, & then lets them select them unless you have something on your calendar at that time.
### two books nick gave me
Designing Your Life
Relevant to: The "what opportunities should I say no to?" thinking you have been doing
Quote:"Designers imagine things that don’t yet exist, and then they build them, and then the world changes. You can do this in your own life. You can imagine a career and a life that don’t exist; you can build that future you, and as a result your life will change. If your life is pretty perfect as is, life design can still help you make it an even better version of the life you currently love living."
Competing Against Luck
Relevant to: Your search for a problem for Configurator to solve.
It's basically an ode to concentrating on the problem, not the solution. I think having a framework might help separating the configurator you want from the search for the real market place problem Configurator can solve. If not, it's still a good book ;-)
### blog of language guy
https://marijnhaverbeke.nl/blog/  
### a PEP
https://www.python.org/dev/peps/pep-3118/
### Nick Configurator
https://www.eetimes.com/document.asp?doc_id=1333422

^^ configurator as lego building block combiner for "silicon compiler"?

_
### QL John
nt to chat with our neighbor John to get interview tips since he hired thousands of them. Man, did I learn so much from him! We should hangout with them more often! He actually successfully exited few companies. He sent this link to me since this girl is one of the investors of the company I am going to interview with, and I like her summary about machine intelligence:

http://www.shivonzilis.com/machineintelligence

_
### Catholic mariage stuff

THANK YOU Daniel and Qingling (TL)!

Our answer to this form will be included in the Answer Key to your next assignment.

Click here to proceed to: God's Plan for Humanity.

or copy/paste this link into your browser: https://web.memberclicks.com/mc/quickForm/viewForm.do?orgId=cmp&formId=23209
An email has been sent to your inbox with this same link to allow you to get back to this form later. 

You cannot access assignments directly from the web site, you need to go through the links we give you and then enter your username and password.

After you submit the God's Plan form, your instructor will review it and upload your answer key onto your profile where you will retrieve it within three business days max.


Please do not forget to send a picture of you to your instructors! It really helps them to get better acquainted with you. 
Add info@catholicmarriageprep.com in the cc of your email to them if you wish to be on our website (Website's gallery or reviews).  


Our offices are open Monday through Friday 9:00 a.m. to 5:00 p.m. Mountain Time. We are closed on weekends and Holidays.

For questions, click here to contact us at any time or call us toll-free at 1-866-425-7193 during business hours.
### Long shannon lessions learned
long article on lessons learned from writing a biog of claude shannon. some gems here: 

> https://medium.com/the-mission/10-000-hours-with-claude-shannon-12-lessons-on-life-and-learning-from-a-genius-e8b9297bee8f 
### haddick
DanO, this is a long essay that I think I will summarize and modify to meet our needs. Don't spend the time on it unless you are really curious ;-)
The article is a pretty close description of the problem we had scaling at Sierra Nevada Corporation and the function that I turned the VP of Technology role into: Product Management. This is what I was trying to describe in our early conversations. It also explains why it is hard for a startup to see the value because the function is distributed around a tight knit team in a typical startup. We are not a typical startup and my intention is to better define the Program Management need and fill it. 

http://organizationalphysics.com/2015/10/15/rethinking-product-management-how-to-get-from-start-up-to-scale-up/

_
### Books
-- [happiest baby on block](https://www.amazon.com/Happiest-Block-Revised-Updated-Second-ebook/dp/B00TCI48AE/ref=pd_sim_351_4/131-5534497-5703418?_encoding=UTF8&pd_rd_i=B00TCI48AE&pd_rd_r=c4d50b43-6ef5-11e9-a963-6f3949decc2c&pd_rd_w=oWByc&pd_rd_wg=aiRQV&pf_rd_p=90485860-83e9-4fd9-b838-b28a9b7fda30&pf_rd_r=NM2GPAV2AJC9DC7D5EPB&psc=1&refRID=NM2GPAV2AJC9DC7D5EPB)    (Harvey Karp 5S DVD)   The Happiest Baby On The Block video
-- Brain Rules for Baby by John Medina
-- How to Talk So Kids Will Listen and Listen So Kids Will Talk  by Adele Faber and Elaine Mazlish
### Logic puzzles:
https://www.folj.com/puzzles/#three


