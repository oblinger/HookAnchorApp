...
^gnd, ^head, ^ns, ^body(nope first arg is tuple), 
...
# ### MATH  - UNIFORM STRUCTURE ###
## === GRAPH - GRAPH MODELS ===
### _

**TL;DR.   Uniform Graphical Models are graphs with possibly-ordered, possibly-labelled, possibly-directed edges.**
...
**GRAPH-TYPES THESIS** -- These 11 vertex attributes form a power-spiral ring underlying all computation.  (e.g. The powerset of these combinations are useful enough that engineers can elegantly build from these semantic combinations without extension or refactoring them.)

**ADJECTIVE NAMINGS**:
	Ordy, Keyed, Mapy, Listy, Sety, Dagy, Taxy, Fixy, Finy, Boundy,

These alternate names are expressed as short adjectives allowing one to parsimoniously name elements in the powerset of combination types:  e.g. one can say the "Source Code" data type is fixy-boundy-ordy-mapy data.

**NEGATION NAMINGS**:
Vertices may also be labelled at NOT satifying some combination of these 11 properties using:
	Bagy, Unkeyed, Relational, Multiset, Circular/Graph, Muty (mutable), Infinite, Unbounded, Directed
...
### --- OPERATOR DETAILS ---
#### -- path_get --
##### - CHOICE - path_get create parents
...
#### --- IDX ---
	...
	**def class** Place(BASE, VALUE):
	...
	_
#### --- LEN ---
	- Returns UND if a unit is an ATOM
	- Returns an Int or INF if a unit is composite
	- If unit is a @lst then 
#### --- ITR ---
#### --- MET ---

	**MET** -- The _meta operator (MET)_ provides extra (non-structural information) about a unit.  It does this by returning a structure containing this information.

	Specifically it returns the unit:

	   meta( gnd: grounding, head: head, ns: namespace )
...
#### --- SET ---
### --- semantics ---
- GET -- returns unit indicated by triple 
			(assuming functional relation)
#### SEMANTICS DISCUSSION (mappy & listy)
	...
	- expr_args	scans 
### --- ADD OPERATOR ---
thnk: what is the unified semantics?  what is listy?
### --- CONTAINER OPS --- 
#### update 
Iterates over 
#### copy
...
_[[Maybe this section goes away.  Both type and grounding are defined in MATH, then perhaps they are only extended in Functional section??]]_

**HEAD** -- ??? a identifier "type-like" indicator associated with each unit.

**ISA** -- 


of the claimed type a unit's structure.
just basic definition here
main stuff under LWL
## === FNS ===
### >> PLACE <<
### -- access --
### -- assign --
...
**def type** _PatchSpec_(K: Ident, V: Spec) :=
	U.patch( U."->"($K, $V)... )
...
Lang Spec is a "Language Writing Language" -- a collection of uniform constructs aimed at writing the parsers, macro-expanders, and transpilers required for creating new Uniform DSLs.
_
## === TEXT ===
...
**TL;DR.  Uniform Boot initiates execution of some target language based only upon a unicore specificaton of that language.**
...
**BOOT BLOCK** -- A _boot block_ is a chunk of code delineated using a boot marker that indicates the sofware language of the code chunk.

**BOOT MARK** -- A _boot mark_ is a specific pattern of characters that indicates and delineates a boot block.  The unicore bootmark matches the following regex:
    /^\W*-~-\s+(.*\S)\s-~-/


	A Uniform language block has the following format:
...
### --- NAMING RULES ---
#### _
- ALL_CAPS -- Constant value names
- CamelCase -- Specifiers (Class, Type, Interface, Template)
- snake_case -- Placenames (Local/Global/Arg varname, )
