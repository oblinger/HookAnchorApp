...
### --- THE "WHAT IF NO TRADEOFFS" INTRO ---
#### A PROGRAMMING PARADIGM W/O TRADEOFFS?!
##### No tradeoff intro v2

Different programming langauges are better for different things.  They make incompatible tradeoffs, and thus cannot be combined into some kind of single best "super" language ecosystem.
...
-- a _**forking split**_, (which are generally "instituted by [wo]man").

An _intrinsic_ taxonomic split is one that is tied up with one or more important properties that are (1) inherent in the underlying mathematics, that are derived from (2) properties of the physical world, or (3) properties inherent to all von Neumann machines.

By contrast a _forking_ taxonomic split is one that is NOT tied to such intrinsic attributes of mathematics, our physical world, or properties of the von Neuman machine.

One can distinguish a forking split by the parallels one can draw across the split itself.  A forking split will have many parallels between the operators and programs found on each of the splits while the intrinsic split will generally result is alternatives that do not map onto each other in practical and natural ways.

For example the notion of an bounded integer is _intrinsically_ distinct from that of a finite set.  There ARE both likely belong to common parent categories (for example, both are a kind of transmittable data and might share common operators associated with data transmission). But at the level of integer itself, the natural operators (e.g. increment, decrement, etc.) are different from those for a set (e.g. membership, etc.), and programs written which use integers don't have natural parallels for programs that compute the same things using sets instead.  Each of these notions has a collection of interlocking properties that do not translate well or at all to the other.  (e.g. increment, addition, etc.)

At the other extreme are two data formats for encoding limited precision integers on von neuman machines: big indian verses little indian format (the first put the most integer significant digits in lower addresses while the second does the opposite.  In this case nearly all operators that apply to one split also apply to the other, and programs written for one split will have a parallel on the other.
...
Even if one might theoretically avoid unforced tradeoffs making progress in this way seems entirely impractical.

Perhaps that would be true if we did not have several powerful techniques at our disposal to drive this progress.


80/20 OPTOMALITY -- Avoiding tradeoffs seems so impossible since there are many dimensions that one might want to optomize, and they often tradeoff with each other.  Any approach we choose in service of any given dimension is likely to have negative impact on multiple other dimensions.  
How can there be NO TRADEOFFS?!?

Most dimensions have diminishing returns and clear regions of disasterous consequence, thus there are often many ways of dodging any given dimensions worst effects, and less advantage in going beyond its first rounds of benefit.
Thus 80/20 tradeoffs often exist between competing dimensions.
...
UNIFORMITY THESIS -- The uniformity thesis is that it is possible and practical to embed all capabilities of all software langauges into a single ecosystem such that most any advantage provided by any of these langauges is also afforded by this ecosystem.  Further these individual advantages may be "mixed and matched" in most any combination that to the degree permitted by the inherent nature of the underlying concepts.
