...
DL consideration categories
-?-
DATA -> MODEL -> TRAINING -> OPTIMIZER  -> TESTING
STABILIZATION, VISUALIZATION <!--SR:!2025-08-29,1,194-->
...
VISUALIZATION: Monitor loss, Visualize <!--SR:!2025-08-29,1,193-->
...
Distributed Training <!--SR:!2025-08-29,1,146-->
...
This is particularly useful when memory limitations prevent large batch sizes from being used. <!--SR:!2025-09-08,11,226-->
...
Model Parallelism: When a model is too large to fit on a single device, model parallelism splits the model itself across multiple devices, enabling larger models to be trained. <!--SR:!2025-09-02,5,234--> 

More INFO
Work Split:
  - Each GPU gets a different batch of data
  - All GPUs have complete model copy
  - Forward/backward pass on different data
.
  Weight Updates:
  - Synchronous: All-reduce gradients after each batch
  - Updates shared via NCCL/Gloo every iteration
  - All models stay in sync
.
  Example:
  # 4 GPUs, batch size 32 per GPU
  # Effective batch size = 128
  # Each GPU processes 32 samples
...
last line
 ...
 2. Model Parallelism
  ...
  - GPU 1: Layers 0-5
  - GPU 2: Layers 6-11, etc.

  Weight Updates:
  - Each GPU only updates its layers
  - No gradient sharing needed
  - Communication for activations between layers

  3. Pipeline Parallelism

  Work Split:
  - Model split into stages across GPUs
  - Micro-batches processed in pipeline
  - Reduces idle time

  Updates:
  - Gradients accumulated over micro-batches
  - Synchronized at pipeline flush
  ...
  Updates:
  - Only communicate needed parameters
  ...
  - Parameters synchronized as needed
  ...
  Asynchronous (Less Common):

  - Updates happen independently
  - Can cause convergence issues
  - Rarely used for Transformers

  Communication Patterns:

  All-Reduce (Data Parallel):
  - Ring all-reduce or tree all-reduce
  - ~O(2×model_size) communication per iteration

  Point-to-Point (Model Parallel):
  - Between specific GPUs
  - For activation passing

  Practical Example (PyTorch DDP):

  # Each GPU runs this
  for batch in dataloader:
      # Forward pass
      outputs = model(batch)  # Different data per GPU
      loss = loss_fn(outputs)

      # Backward pass
      loss.backward()  # Local gradients computed

      # DDP automatically does all-reduce here
      # Gradients averaged across all GPUs

      optimizer.step()  # All GPUs update identically

  Key Takeaways:

  1. Data Parallelism: Updates shared every iteration via gradient all-reduce
  2. Model Parallelism: No weight sharing, only activation passing
  3. Hybrid approaches (3D parallelism) combine multiple strategies
  4. Communication overhead is major bottleneck at scale
  5. Modern methods optimize what to communicate and when

  The specific strategy depends on model size, GPU memory, and network bandwidth. <!--SR:!2025-07-16,1,250-->  
...
{{LAMB - Layer-wise Adaptive Moments}} <!--SR:!2025-08-29,1,188-->
