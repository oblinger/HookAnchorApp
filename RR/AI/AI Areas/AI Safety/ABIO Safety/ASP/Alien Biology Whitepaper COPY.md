...
**Background**: Most tests of agentic AI and LLMs are drawn from various human areas of activity. This has the advantage of testing the *breadth* of AI's learning and capabilities, which are often carefully matched to problems of practical importance. But these advantages generally come at a price:
...
- **EXTENDED INFERENCE**—Existing tests match the case where a human learns for years, then is tested on a small, complex problem. However, this does not match the realistic scenario where a human spends months or years learning while solving a complex task; yet, this is precisely where current AI systems fail. (see METR results).
...
These are all things humans can do, yet each represents hurdles that the current heavy-train-time, light-inference-time LLM architectures have great difficulty with. Thus, Alien Biology promises to shed light on a key gap in our progress toward true AGI. 


## Introduction
Measuring the ability of today's LLM-based systems to perform complex reasoning is often confounded by potential contamination of their training data by problems related to those used in the testing. This problem is especially acute when measuring complex or agentic reasoning, as both require extensive background knowledge. It's not practical to invent entirely novel contexts for each challenging problem.

If only there were some alternate universe that was as difficult to reason about as our universe, but with all details changed such that training on text from our universe afforded no advantage in answering detailed questions drawn from this alternate universe. Then, we could measure an agent's ability to reason about this alternate universe w/o concern that knowledge about the questions we are asking was provided during the LLM construction.

The Alien Biology framework described below is designed to allow us to construct just such alternate universes for testing our agentic systems. We aim not to recreate an accurate model of any particular universe, but to build new universes that are simplified to target reasoning structures similar to those in our own world. This gets to the crux of the complex reasoning w/o wasting effort with needless realism.
...
Below is an idealized model of biology that covers (in a simplified way) nearly any task one might undertake relative to almost any biological topic. This framework can encode low-level functioning within a cell, such as the Krebs cycle, signaling pathways that coordinate groups of cells, the functioning of whole organs like the liver, and up to the highest level of interaction patterns found between socially interacting animals.
...
5. TASK: Templated test tasks, such as "Understand and cure this disease," can be formulated within these synthetic worlds.
6. CONTROL: By controlling the parametric generation of these worlds and tasks, one can fine-tune the testing of various aspects of the complexity of the learning and reasoning task.
...
As with real biology, solving the more complex versions of these tasks forces the reasoner to invent new abstraction layers one on top of another to address the overall task in question. This allows us to construct tasks that might take a biologist anywhere from 5 minutes to 5 years to complete. This kind of assessment, in particular, is nearly impossible to test in an untainted way using native tasks; any naturally occurring hierarchy of abstractions is almost certainly to have been well documented within the text used to train the LLM, making it impossible to assess the system's ability to derive and use those abstractions.


## The Formal Framework Underlying Alien Biology

This section defines the abstract framework we will use to construct our alien biology. Ultimately, the ecosystem and its contained organisms will be encoded as a large JSON structure defining their contents, along with numerous Python functions used to describe the bioprocesses, measurements, and actions that operate within that universe. This formal model is not provided directly to the agentic reasoner; instead, it is used to drive the world in which the agent operates when solving the given task.


An **organism** is represented as a directed acyclic graph (DAG) of organs, with associated metadata for each. We sometimes refer to this annotated DAG as the organism's **physiology**.
...
{"kind": "Substrate", "num": 1,
	{"kind": "Protozoan3", "num": 15000, 
		parts: [{"kind": "energyorganelle4", "num": 65, ...}, {...}]},
	{"kind": "Food5", "num": 300000, 
		parts: [{"kind": "biomolecule_carb", "num": 100}, {...}]},
	...
}
...
Typically, the rate at which each biological process executes is referred to as its efficiency, and its rate formula stochastically controls it at each moment. But more generally, we can formulate each bioprocess as one that accepts a world state and returns an updated world state for the whole ecosystem.
...
**MEASUREMENTS** - Of course, one needs to be able to take accurate measurements of a biological system to study it effectively. In our case, this is simply a function that takes a world state as input, along with any required parameterization for the measurement, and then returns the measurement's results. The results may be a numeric value, such as the concentration of a biomolecule within a specific type of cell, a sequence of values (like temperature over time), or other types of data output. The measurement's inputs might include a specification of what biopart of the system one is measuring. 

**ACTION**—The AI agent also needs the ability to act on the Alien Biology in some way. This is accomplished via actions. Like a bioprocess, an action accepts a world state as input, along with any parameters required to specify the action fully, and returns an updated world state. The difference is that the parameters for a bioprocess are typically fixed within the subsystems in which they operate. Meanwhile, actions and action parameters are typically independent variables under the AI agent's control.
...
**SKIN** — Each of these symbolic and computational components can optionally be described textually. Providing the AI system being tested with such textual descriptions of the bio system is equivalent to a researcher beginning a task with knowledge gleaned by others before them. Some tests can provide significant textual descriptions, while others offer access to the world to be understood and controlled, and nothing else. The amount of 'skin' provided to an AI agent is one of many dimensions of complexity that can be varied in testing its capabilities.

**BIOSYSTEM** — A biosystem fully defines an alien world in which agentic testing may be performed. As shown above, it can be encoded as a Python module, aggregating the aspects defined earlier: biomolecules, bioparts, bioprocesses, measurements, and actions.
...
**EXPERIMENT**—Using this framework, we can describe an experiment as an intervention script run over a partially understood world composed of an unknown number of bioprocesses. The script defines a combination of measurements and actions taken over a simulated period. 
...
2. Control various dimensions of task complexity in a fine-grained, controlled way without requiring humans to create tens or hundreds of thousands of tasks to test.

We achieve both of these goals by dynamically generating new alien biologies and generating tasks within those biologies. We can utilize the hundreds of thousands of understood processes, molecules, organs, and systems to create realistic universes and realistic tasks within those universes. However, we dramatically compress these hundreds of thousands of processes, molecules, organs, and so on, into a relatively small model for generation. Thus, the nature of biological processes is retained. e. the idea of a cycle of molecules as found in the Krebs cycle, without any specific information about the specifics of that cycle, which exists within a system, what its purpose might be, etc., just the bare structural information with work from. Thousands of measurements, actions, and processes are expressed in Python (using LLM models), and then these structures are distilled into generators of plausible but quite randomized processes. The system retains the structure required to achieve some homeostatic or other goal but is divorced from any of the particulars found within Earth's biology.

Model skin names are constructed similarly. One can distill the mapping from formal function to naming within Earth's biological systems to produce a generator of plausible names for alien biology, which may or may not closely tie to the functioning of each biopart within that biology. Thus, this allows us to simulate conditions with varying background knowledge available to the agent being tested.
...
- The number and complexity of the interactions between the layers of the whole system.
...
By distilling the workings of our biological universe into a generator, we can create counterfactual worlds that vary precisely in the aspect of complexity we are trying to understand or improve within our current agentic systems.
...
2. Testing of counterfactual conditions that do not exist within our world.
   *But can these map onto the key aspects of our agentic systems,  that we hope to test?*
...
We believe the first kind of learning and inference is well measured by existing agentic testing. In this case, we *want* the agent to be exposed to static written forms and then measure the performance that results. Alien biology is not trying to test type A. Indeed, we hope to be relatively isolated from Type A knowledge and instead focus on Type B learning and inference. Type C is out of scope for this first version of Alien Biology.

Splitting the learning and inference task this way allows us to consider these isolation and generality claims more effectively. Alien biology is about ***biology***!  Thus, any generalized knowledge about how processes connect and how one might proceed in testing or understanding such a system is available within the training provided in constructing an agentic LLM. Thus, assessing these systems on how well they learned, for example, to isolate the functioning of a biomolecule or its strategic approach to diagnosing and correcting an imbalance within a system, would make no sense. Such knowledge was available during agent construction. However, note that all of this knowledge falls under type A above. It is not the kind of knowledge that Alien Biology is designed to measure; indeed, much of that knowledge is implicitly embedded in the framing of Alien Biology itself.

Instead, we take that structure as a given and assess how well the agent can apply and reason with that knowledge as structural complexity increases. It's like the difference between being able to multiply two two-digit numbers and being able to multiply two ten-digit numbers. How complex can one's type-II reasoning get before the agent confuses itself and cannot proceed?

It also measures the degree to which the agent can recursively build new abstractions while solving a single problem and then use them layer by layer within that task. We have ample evidence that current agentic AI is incapable of this level of generality, whereas humans are. Humans can repeatedly apply biological strategies to incrementally uncover the functioning of even quite complex, multi-layered alien systems, while (we believe) current agentic AI systems will not.

If true, it provides a unique and parametrically controllable window into the gap between humans and current agentic AI.


**But how general is this testing framework?**  It is all about biology, occurring within a very narrowly defined framing of biology at that. The answer depends on how independent Type A and Type B learning and reasoning are. Suppose they are pretty independent of each other. In that case, one can understand type A reasoning as providing the raw structures, processes, and strategies that the AI agent has to work with, and type B reasoning utilizes those processes to perform its reasoning and learning. If this is true, measuring type B reasoning and learning in Biology provides a good proxy for how these systems will perform in extended type B inference over other domains. In contrast, existing Type A tests from other researchers provide assessments of the generality of these systems across various domains.

We can't be sure about this without testing (which we propose to do with later versions of Alien Biology). For now, we observe that whatever knowledge the AI agent has about biology, it has acquired through the training data provided during the construction of its LLM. We have no reason to expect that such training is somehow differentially better at training bio strategies relative to learning and inference strategies in other domains. If this is true, we expect investigating Alien chemistry, physics, geology, sociology, etc., to yield similar results. A system that is good at Alien Biology will be good at Alien Chemistry, too, assuming that the LLM training data contained a good background in chemistry.
