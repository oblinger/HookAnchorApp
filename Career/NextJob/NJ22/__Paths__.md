
# ### BIG ### 
## 2021.10.24 - Dan Fine KM Summary

BIG TRUTHS
- PRETTY STUCK -
- DREAD OF WORK -
- FEAR I CANT -- Just a bit, but not very rational
- FEAR I WILL HATE IT --
- VIEW IT AS 'DEATH' --
- DAN WORK - I am mostly driven to execute Dan work, not really to
- WHY DO IT?
- DRIVEN - Driven towards Dan Work - intrinsically motivating
- TO PROVE I AM RIGHT - To myself, to everyone, to see if I am right.
- DO WHAT I AM BUILT TO DO
- DO WHAT I AM CAPABLE OF
- NOT (very much) for its outcome
- NOT for money or such
- NOT (exactly) for fame
- TO AVOID DRUDGERY
- LIGHT SWITCH - My interest in dan work can be very intense.
	But it can rotate among several.  (will come back)
- LOST COLOR -
- ESCAPE&CONTROL - I am driven to work, but will feel strong need to swap sometimes
  
  
  
  _
## 2021.09.14 - DAN-WORK - The kind of conceptual work that drives me
### _

There is a certain kind of work (lets call in DAN-WORK) and this kind of work is so compelling I tend to work on it even when it is not rational to do so; in to the night; sometimes at expense of nearly anything else.

DAN-WORK = TRANSFORMATIVE, CLEVER/HARD, INTERNALIZED, CONCEPTUAL PROBLEM-SOLVING and MENTORING


CLEVER/HARD
- HARD WORK CAN BE BORING -- work that is just hard, like solving some huge calculus problem is just drudgery.  There are no insights to be found or tricks to play then it is exhausting and not very rewarding
- CLEVER WORK IS TOO RANDOM/QUICK -- work that is just one clever trick has no staying power, in one flash it is over, or you have no ideas and make no progress forever.
- CLEVER + HARD -- now we are TALKING.  I can play with the right kind of mathematical puzzle for weeks if it is the right shape.
  
  
  TRANSFORMATIVE
- LIFE MISSION NEEDS CONSEQUENCE -- even if I can play with a hard+clever problem for a long time, it feels hollow.  like binge-watching six seasons of game of thrones in one weekend... it was kinda fun a the time, but also anxiety provoking because you are say...  why the fuck am I doing this?
- A TRANSFORMATION STORY PROVIDES THE ANSWER -- when I can construct a story about how solving this problem will somehow transform things for some group for the better in a significant way, then have no such hollow feeling... it it jet-fuel to work even later into the night!
- SOCIAL -- One common and very satifying way for work to be transformative is for the work to be social in nature, figuring out how society can work better is inherently transformative and hard (if it were easy it would already be done!)
  
  
  INTERNALIZED
- NO PAPER NEEDED -- the very best problems are ones that can be considered for days or weeks without even needing a piece of paper -- just pure thinking.
- LESS LIBRARY RESEARCH -- I am ok needing to do some background learning in order to attack a problem.  Still this learning, unless it involves learning clever insights is a bit of needed drudgery work, it is not the main course.
- LESS COLLABORATIVE -- I am ok problem solving as part of a team, but I like moving fast, and I like the thrill of the solving itself, thus collaboration can be a bit of a speed bump for all of that.  It is ok, and can even be energizing when my partner is complementary and sharp/fast.
  
  
  CONCEPTUAL PROBLEM SOLVING
- PROBLEM FRAMING -- Often the most unconstrained (and thus hardest/cleverest) part of solving a problem is properly framing it in a way that it CAN be solved in a good way.  This is very satisfying work.
- EDGE OF FORMAL AND INFORMAL -- Often the at the fringes of what is known and what is properly framed is where the exciting action is at.  It is close enough in that one might make the transformative difference by properly framing the work, but it is not so far in that the proper framing is easily obtained.  Often this work is called "conceptualizing" a problem.  I freaking love it!
  
  
  MENTORING
- Advising
- Vision --
- Details & Tracking & Plodding -- not so much
  
  
  EXAMPLES OF DAN-WORK:
- BLOCKCHAIN - Building a POS block chain as robust as today's POW chains are
- ELDERBOT - Building an automated human-transport robot for the elderly
- TEAMSAVER - Identifying and overcoming limitations that make remote work less effective than in-person work
- ENGINES-OF-CONTROL - Building a conscious computer
- NANO-SCALE-PROGRAMMABLE-MATTER - How to build programmed 3D-nano structures
- AEOLUS-ROBOT-ARCHITECTURE - Designing the high-level APIs and architecture
  
  
  
  NEGATIVE EXAMPLES
- BORING DETAILS - I desperately want to hand off
- POLISHING - Polishing text describing an idea
- MEETINGS - Being in meetings once all the hard stuff is resolved
- TRACKING - Tracking work being done
  
  
  
  WHAT MIGHT FIT ME?
- BUSY BEE - helping with the framing & large scale solutioning across many hard tasks.
  
  
  _
### 2021.11.21 - NINA WORK

HARD - Trying to prove theorems about privacy is very hard.
DELEGATE - Working with student means they do bulk of the tedious writeup.
EXPERT - Working near experts means that my work is efficient

_
### - Conceptual Work

I am positively driven to think about
- the detailed conceptualization of a clever solution to a big-impact problem.
- Love just thinking, 
  
  
  DONT LOVE
- Don't love doing the library research
- I dread coordinating it.
- I resist doing it on a schedule, or for many hours at a time.
- I dread working on more boring parts.
- I dread working on parts where cleverness is not needed.
  
  _
### - Technical center
Clear that my work must have a strongly technical center.
Possible centers:
- Applied ML reseracher
- Robotics project lead
- SOLVE -- dig into range of disparate problems and apply deep tech to them
- MENTOR -- Work as mentor to direct technical work to be done
  _
### - Procrastination
- As soon as I work, I resort to procrastination by 
	organizing / processing easy stuff
- Emails, Medium/Blogs
- Take Nap (I physically feel sleepy in a quick way)
  _
### - Uniform/Marcel
- IMPORTANCE -- feeling that the idea has potential for large impact
- UNIQUE -- feeling that idea that is not generally known, or solved
- PURE THOUGHT -- Struggling to understand/solve using pure thought
-
- CLEVER SOLUTION --
- EFFICIENT SYSTEM -- love thinking about how to make systems efficient
  
  _
## 2021.07.31 - The Past and The Future - Summary for Nick

THE PAST
- MISSION #1 -- Achieve some larger-than-self impact from one of my many ideas
- MISSION #2 -- Spend my time working on diversity of challenging/fun ideas
- SUBGOAL -- Create lifestyle biz needing few hours to support mission 1 & 2
- DIAGNOSIS -- My execution was split & not well informed. Better: do #1 alone
- OUTCOME -- Only some success on #2.  Bit of money & experience too.
  
  THE FUTURE    (analysis of long terms options)
- CONSULTING -- Probably not enough money for long term 
	(maybe on-par w/ google if fulltime, but that would suck more long term)
	(could be used to stretch a medium term situation longer)
- AI SAFETY -- Not enough money for long term; and not a forever mission
- STARTUP -- Too risky to consider now unless I am dying to do it
- LIFESTYLE BIZ -- Too slow & too risky now.  
	Jason's gig is best bet in that direction, but still risky & slow.
- BIG CO -- Likely this is the only real option for long term
  
  THE PRESENT   (medium term window 1-2 years)
- TOP COVER -- Use AF payout as 'top cover' to wife for gap before big co
- TOP COVER -- Get some affiliation as 'top cover' for future employer
- GOAL:
- ACTUALIZE -- Get stuff 'out of my system' before big co jail
 		==> into form where I can claim it as protected IP
- MOON SHOT -- Take long shot on 
	  
	  
  SATISFYING WORK (more items below are better, but not all are required)
- HARD THINKING - Not just execution, solve hard-to-think-thru technical tasks
- CLEVER DETAILS --
- HELPS THE WORLD (NOT MOSTLY JUST BIG CO)
  
  
  EXAMPLE AWESOME TASKS
- TECHNICAL PROBLEM FRAMING -- Hearing about a novel technical challenge, and thinking thru how one might address it at a detailed technical level.
- e.g. how to model task in order successfully apply ML.
- e.g. how to architect an approach
	  
  _
## 2022.02.15 Choice Space
FORK: Startup, AI-guy

CHOOSE: PAY, AREA, VALUE/ROLE, ORG

PAY
- >200K 
  .
  - Startup
 

POSSIBLE AREAS:
- Applied AI
- Deep Learning
- Robotics
  . 
  - Knowlege Management
  - Blockchain	

ROLES:
- TL - Team leads

ORG:
- MY STARTUP
- FAANG
- MID/LATE STARTUP
- RESEARCH INSTITUTE / UNIVERSITY


KEY VALUES PROVIDED:
- Lead technical team's choices in delivering a vision
- Reducing Technical Risk


_


# ### PATHS ### 
## alternatives
### TECH LEAD
- Lead small team or moderate org in product creation
- Lead strategic vision and technical decison making
- Team hiring
- Solution architecting

_
### DL RESEARCHER
- 



_
### SOCIATAL MISSION



_
## -- Big Arcs --
- **AF** -- Drive consulting to >$350K Nick, >$200K Dan.   Current Aim: Embedded
- **PRODUCT** -- Current: RemP    Others: Insurance
- **CORP** -- Back to reserachy + leadership in: 

_
## -- But What Do I Want? --

-- love structuring the solution.
	> can that work at amazon

_
## -- Specific projects / paths --
### - remote presence -
- Outline MVP idea, get feedback
- Ask MEL, ????
- Xxxx
_
### - autonomy -
- Ask Berkeley - what is cost/time to gain approvals
- Ask ALS - who would pay, what would have value
_
## --- ALTERNATIVES ---

BIG-CO -- But then what do I focus on?
- TEACHABLE ML
- ROBOTS
PRODUCT
- PRESENCE
- AUTONOMY
- FORUM
LIFEPROJ
- UNIFORM

_
## === ALTERNATIVES SUMMARY ===

- FAANG RESEARCH
	- TL->RESEARCH - DL - NLP
	- TL - ROBOTICS
	- 
- CTO - level of stress/uncertainty
- 




# ### LOG ### 
### 2022-03-13 - inside google

**INSIDE GOOGLE**
- **TECHNICAL WORK** -- Happiest if I would describe my work as TECHNICAL.
	- **SOLUTION FRAMING** - working with stakeholders, engineers, etc. to help frame basic approach, outcomes, paths, etc.
	- **ARCHITECT** - working at the next level in leading API/architecture etc
	- **APPLIED RESEARCH** - working AS an applied researcher trying to develop new ways to solve important problems.  (programming & experimentation)
- **AREA** -- ML and (AI + Robotics)
	- ML CENTERED DEPTH -- PhD-IC-level abilities only in this area.
	- ROBOTICS / AI BREADTH -- Ability to ideate and manage across wide range
- **PRODUCT/INNOVATION ORIENTED JOB** --
	- +++ Love to be an ML expert framing and mentoring multiple projects
	- Happy to manage portfolio only if it involves significant tech thinking
	- Could be tech lead for a single project too.
- **VALUE-ADD**:  TECHNICAL LEADERSHIP  using  ML 
  
  
  **KEY ACTIVITIES**
- Refreshing ML skills
- Calls (Ben CEO-leaving Amazon; Nina Researcher @ Amazon; Praveen @ Google)
	- Exploring ML-Research roles.
- talk w. some recruiters
  
  **NARROWING THE APERTURE**
- My center is ML (with spokes out to: AI, Robotics, Leadership+, Research++)
  
### 2022-03-13 - zzzz
PATHS
	+++ MEGA CORP (IC Specialist, Technicalish-Manager, Researchy-Group)
	.5y to 1.5y before starting.  (preserve soul, retraining, raise Eli)
	++. FUNDED STARTUP (R-ZERO)
	heads down 100% focus for several years  (not Aeolus level insanity)
	-- synergistic w mega-corp path, good pay, synergistic w/ AF, ML
	x.. MY OWN STARTUP (KM-CO, Teamsaver, Marketplace)  customer+funding+partner
	xxx CONSULTING
	xxx AI SAFETY



### 2022-03-09 - Post Amy Kruse Thinking
#### ROCKS
##### THINKING FAIL
- My love of thinking deeply about problems is NOT a good path for founding a startup.
- I CAN found, but it needs to be done in very different way
- My ideas of Uniform, AI-safety, and Knowledge Management are all POOR starting points for a new business.
#### DEEP TECH
- Happiest when I am learning and problem solving around hard ideas
- Like to read/learn
- Like to brainstorm how to solve something
- Like to advise / mentors others around such ideas
### 2022-02-22 - Latest Thinking

**PARTIAL BREATHER?** -- Consider breather.  Already have good retirement position.  Significant lift to get to GREAT position.  Thus consider lower income paths now, if strongly preferred.

**MUST LOVE IT IF CTO/VC** -- Energy, but must LOVE it.  These paths seem notably harder, but could be fufilling, and lucrative.  Consider them if inspired by the work.

**APPLIED-AI** -- Utilize AI & Mgt background to lead in a more researchy role. 





Talk about Deep.  
- several times you said, he could be part of our coaching.
- gives talks maybe I 


LIGHT HEARTBEAT / HEAVY TACTICAL
- Word smithing
- Outreach strategy
- Retooling strategy & validation


_
### 2022-01-04 - My purpose

- NOT MONEY -- It is now too late to start a business in order to make FU-money.  But will have enough to retire on in a few years if RZ does well.
- MIDWIFE -- Bring an important idea into existence... this is my purpose.
- LOVE&LIVE -- 
- THINK HARD -- I love to think hard about a valuable thing.  
	This is my passion that can drive effortless action, but it needs to somehow tie to something of import
_
### 2021-11-14 - Ikigai - my "passion" - IDEAS

- LOVE - Technical Creation
- NEED - tech-jobs
- PAID - tech-job
- GOOD - 


My Guiding Light:
- FUN - Technically hard.  Envisioning/Leading.
- IMPACT - 
- PAY - 


DAN-WORK = TRANSFORMATIVE, CLEVER/HARD, INTERNALIZED, CONCEPTUAL PROBLEM-SOLVING and MENTORING


SKILLS: Problem Solving
I GET: Cool problems
Work Env: 


=== ===

- ENABLE HUMAN COLLABORATION -- Enable Human Ownership
	- SHARING CODE -- 
	- REMOVING CONTROL-BASED BLOCKING -- 
	- INTEGRATE DISPARATE INFO ORGANIZATIONAL KNOWLEGE -- 

- AI - SAFETY
	- FUTURE PREDICTION
	- EXPERIMENTALLY SHOWING 

- ROBOTICS

- TEAMSAVER

_

_
### 2022-01-20 - PATHS, spiritual dimension

- BOTH - What ever path I take, I need to satisfy two key needs:
	TECH - Need to be creating something hard w/ my mind.
	BENEFIT - Somehow I need to be doing something of value.

FAANG Eng
- As a FAANG tech lead I could leverage my past.
- Earn good money.
- Over time move towards more researchy projects.
- Publish stuff on the side that has societal value.

MIRI --> FAANG
- Spend a focus year publishing EOCA results, then move towards a FAANG job



_
### 2021-11-25 - PATHS TABLE SUMMARY
			Joy		Tech	Mean	Pay		Diff	Skill	Idea
GAI			++		+++		+++		>6		2y		++		++
KMCO		+++		++		++		?		10%		++		++
R-zero		+		+		+       3+		0y		^		v
GOOGLE		+		+		-?		>6		0y		++		+

CTO			-		?		-		>2++	1y
MyCo		

Robotics	+		^		o		>6		0y		Faking it??
TeamSaver	++
SportsV


NON-PAID
Forum		++
EduCorp		+


##### - terms -

SCALE:	+++ ++ + ^ o v - -- --- 

JOY:	How much will I just do it w/o prompting
TECH:	How technically & algorithmically hard is it
MEAN:	Has meaning for the world
PAY:	How much can I can get paid doing it? in hundreds K? '+' = equity
DIFF:	Difficulty in years or percent success
SKILL:	How differentiated am I for this kind of work
IDEA:	Dan-idea driven


GAI:	Writing about and building GAIs.
KMCO:	Starting with treesheets, building a biz that serves individuals|corps
RZERO:	Be a VP at R-zero
TL:		Software Engineering Team Lead at Google or other FAANG

_
#### NARRATIVE SUMMARY

- **TECHNICAL MISSION** - Main job must have some kind of technical mission.
- **ROOM** - Main job should have room for the rest of life.


- Treesheets is the only dan-idea batswing I could execute now.
- Pause->Google works but does not add too much value
- Rzero is better than Google & CTO



PATHS

**RZERO** - 
**FAANG+ML** - 
**TREESHEETS** - Validate idea.  Build/sell the treesheet productivity tool. +MKT
**AI-SAFETY** - Validate agenda.

**UNIFORM** - Validate idea.  Forge a community building/using uniform computation. 

**FORUM** - Validate idea.  Build non-profit & community of consensus seekers.
**EDUCORP** - Validate idea.  Build non-profit 

CRYPTO TRADING - 




_
#### RZERO 

- HEADS DOWN - Go heads down for two years.
- EARN >$1M/yr
- RETIRE after 2 years


#### FAANG+ML

- SUMMARY -- 
	- FAANG -- Join a FAANG as a SWE team lead software manager.
	- FIVE YEARS -- work for five years until 62 then retire.
	- WEEKENDS -- spend some weekend time on a heavy hobby or two.

_
#### KMCO

- TRAJECTORY -- The chances for the KMCO business is not high enough by itself.  I need to commit to the area even if that fails, to keep driving KM in other ways.
- 

_
### 2021.11.25 - PATHS MISSION

MISSION:  What do I want my life to be about?
- ROOM - To explore technically hard/interesting ideas & live life w family.
- LAUNCH - Launch one or more new ideas that benefit/transform the world.

LAUNCH TARGETS
- UNIFORM: 	Academically published + KM tool + Active community +  Mkt
- FORUM:  	
- EDUCORP:	X

_
### 2021.12.01 - PATHS cash

1/4.5% * 12mo = 266 multiplier

$200K = $12K (after taxes)  Joint gets $12K/mo now.
$400K = $200K + $200K	= (22K)	10K/mo 	= 2.6M
$500K = $200K + $300K	= (27K)	15K/mo	= 4.0M
$600K = $200K + $400K	= (32K)	20K/mo	= 5.2M



$3.3M Cash Today + 66K shares
$900K pretax. so assume $3M total post tax

RZERO VALUE
66K @ $13/share = $858K
80K @ $13/share = $1040K / 4 = $260K

	Net3M	AFsale	Salary	2yr
1x	$3.8M	$858K	$260K	$5M
4x	$6.5M	$3.4M	$1M		$9.5M
8x	$10M	$6.8M	$2M		$12.5M

_
### 2021.11.30 - PATHS GOOGLE vs R-ZERO vs UF-now

			Google	Rzero	UF
Pay				  	  X		--			500+?200  vs  300+($2M)    250K-bseries
Passion				  X		+++
Learn				  X		-
Room		  X				+++


80K shared @ $13 = $1,040 /4y = 255K/y * 8 = $2M/year (assuming B * 4 * 2)
_
### 2021.11.09 - PATHS (Luis note)

LATEST THINKING
- FLOAT -- Crypto
- GOOGLE -- 
- R-ZERO -- Best easily accessed CTO role, but is double dipping. VP+ ML-
- UNIFORM -- it is a big hobby.  requiring room, but it is not a job/life
- TREESHEET -- moonshot; would require validation, prototype, funding <20%


Luis,
You got me thinking about how to validate my treesheets KM idea before prototyping it.  I think it is a very wise path, just not the most natural path for the more engineering minded.  Let me take a shot a what this might look like, and see what you think:

I pick several specific info management 
_
### 2021.04.19 - DANO PATHS
#### _

- MKT - If it could work
- ML - Corp, Jason, Research  (Backup plan)
- KM - 
_
#### PATH - MKT
- EXCITING -- Crypto & Social Systems are super cool to me
- BIG JUMP -- Not leveraging my ML background

- RESONATE -- Vision; Algo hacking/learning; Social Experimentation
- BREAK -- Build community

WHY:  We are
- owned by our social networks, (facebook, instagram)
- not paid for content (medium, yelp)

_
#### PATH - KM (Corp or Personal)
- See CorpKM problems

_
#### PATH - Basketball
#### PATH - DEEP TEACH
> Robotics

- Deep Visual-Temporal-Functional Models
- Integrated with NLP
- Voyeristic Learning - task expert transcripts

_
#### PATH - TEAM SAVER
- NO MOTE -- Michael: don't worry about it


##### Competitive landscape

DIFFERENTIATION:
- Always on (like slack)
- Audio-only
- Entirely passive comms
- No hands

FEATURES:
- FRICTION - Low friction call (other person does not need to answer)
- AWARE-AVAIL - Awareness of the availabilty of others to talk
- AWARE-APP - Awareness of what app the other person is running
- LOOSE - Ability to hear multiple conversations at once

TANDEM: Share screen/cursor; Friction, Aware(avail&app)
TEAM_FLOW: Aware(Avail), Loose comms
GATHER.TOWN: Friction, Aware(Avail)

_
#### PATH - AF Specialty

_
#### _
STYLE	-- Lifestyle biz <25hr/wk
RAISE	-- 
C-RES	-- Corporate Research
C-MGR	-- Corporate Manager
C-LIFE	-- Corporate Lifestyle <25hr/wk


FAANG -- The mostly likely path is an ML job at FAANG
AF -- AF is probabably ending soon
BRAIN -- A long shot is to work on bootstrapping of K structure
EMPTY -- Do not see the business I will start

$500K/yr = $2M / 25% chance of winning = $8M


CORP KM -- 
MKT -- 


_
#### 2021.03.28 - Big picture next step ideas / pulls

> >> IDEAS WITH HIGH PULL <<

	**Conscious AI Vector**.  NLP + DeepLearning + Vision
		Post Doc; EIR; 
		--> Tactical move to work w/ Jason
		--> Tactical move to do post doc
		Ultimate Aim

	Corp Info.

	Value Markets



	_
#### 2021.04.14 - Compass

N	--  SPIRIT		--	My spiritual life
NNW	--	FAMILY		--	My family life
NNE	--	FUN/SOCIAL	--	My social life

W	--	WORK		--	My work
WNW	--	
WSW	--	CHEW		--

E	--	NEXT		--	NextJob

S	--	DAILY	My daily/weekly habits
SSW	--	HEALTH	
SSE	--	

_
#### 2021.03.03 - Dan's North Star

BUILD FROM STRENGTH - None are mandatory, but more are better
- AI/ML/DeepLearning
- Embedded/Device
- Energy
- Social Knowledge


SMALLER 
- CHEAPER -- Target bootstrap upto $3M in funding 
- WRENCH -- Target solutions that may be adopted "a la carte" within larger value chains

_
#### 2021.02.24 - Dan's Futures Ranked
	- CONFIGURATOR PRODUCT -- 
	- AF PRODUCT -- 
	- GOOD JASON SITUATION -- >$150K
	- GOOGLE PROJECT LEAD -- 

	north star:
	- FERRIS -- Think big thoughts, side projects, >$200K for 15hrs/wk of ok work
		- DRAWN -- I am drawn to certain ideas, but they are hard to commercialize
	- TECHNICAL MISSION -- Interesting technical mission to solve; some leading?


	UNIFORM   (less than 50/50)
	- Code Markdown -- 
	- Value Markets -- 
	- LAL10 -- 

	We know the challenges. They are big. So do you want to take a run at uniform?
	=> yes, but not a full throttle run at the ground. need feedback I may never get 


	- Jason's idea is pretty fast to test & could scale big
	- 30K teams.  10% penetration.  $100/mo profit  =  3.6M per year  For schools


### 2021.02.17 - Dan's Mind & North Star
#### VALUES
- **AUTONOMY** - Freedom to contribute w/o concern for cash
	- FRANKLIN - Accomplished much because he didn't need to chase $.  99% do
	- OBLIGATIONS - Family
- **FRONTIER++** - Make a Dan-idea real.  
	- CMKS - Componential & Minimal Knowledge Systems
- **FUN** - Techno/_Conceptual_/Abstract Prob Solving; 
	Systems inventing; Big Ideas; Impact; Diverse
_
#### NORTH STARS
- CMKS (Corp/Individual KM; open-src data-bricks)
- DEEP LEARNING DEVICES
- SMART ENERGY
_
#### STRENGTHS
- Articulate / Persuasive
- Quick conceptual competance
- Creative problem solver
- Sprinter
- Raw conceptual processing power
- Single variable optomization
- Very analytical
- Span the stack from very high level to very low level
- Human optomism; trust; works 
#### WEAKNESSES
- I'm a dreamer  (Over scoping; going too big)
- Sprinter
- Over application of Single variable optomization
- Confusing; story changing too fast
- Big Decision Paralysis
- Time Thief for others; Time theif for self


##### DEATH OF THE AUTONMY DREAM - FRANKLIN / FERRIS
- _**Franklin**_ did much - he established printers w/ someone else running it
- _**beyond-cash**_ -He wasn't the richest; importantly wasn't trying to get richer
- _**did much**_ -Did not rest!  He accomplished what his innate abilities allowed. 
- _**99% cant**_ try.  Could do much since no one else is there.
- _**lost?**_ - beyond-cash used to seem possible for me
- _**long shot**_ - Now is seems more of a long shot
- VALUES:  fits w. dan ideas; impact

##### Big Rocks
- CMKS >30-40% - Take shot on product goal
- AF-AUTONOMY >40-50% - Take shot on lifestyle goal
- VISIO - Try it first; leave if sucks
- GOOGLE - Train up for specialty so I can have a little fun


1. Is being a painter my number one
2. x
3. Not Jason vs. 


##### ROLLUP
- **ALL IN MCKS** -- It is a risky all-in bet right now.
	AF very unstable now; might be around afterwards.
	Else go to Google if all fails.
- **AF-FIRST** is on borrowed time. if I am going to do uniform is must be soon.
	If doing AF lifestyle first it will be years before stability
	If doing AF lifestyle at same time, one will suffer significantly
	- Requires 10x revenue for success than Visio
- **Visio** seems a higher probability success (than some unnamed AF approach)
	=> Negotiate Equity w. Jason / Verify actuality / 
	=> Drive hard to build solution (Small time for CMKS)
	=> After 2 years kick off side project in MCKS
	=> After 4 years drop to fewer hours & crank on CMKS  (trusting in equity!)
	??? - talk candidly w. Jason about side gig @ 2Y
	??? - side gig, edit equity if raise on it.
- **Google-Next**, get stuff published to protect IP & get feedback
	Then to Google having an MCKS "hobby"
	Retire after 8 years to work on MCKS or other stuff
- **Ditch Autonomy Dream -- No MCKS; No Franklin** 
	Visio: Fun leadership, Learning techical, 
	Google: good pay, (goof around w/ info systems on side)
	GAI: Go deep on gai, then Google self


##### ALL-IN-MCKS - What would this look like?
- AF - work many hr/wk to keep pushing for AF stability & AF-lifestyle. AND
- FEEDBACK ON
	- Presentation & Detailed Examples; Feedback on: 
		- Value Markets; 
		- Corp KM; 
		- OpenSrc DataBricks
	
- COMMUNITY
	- Smallest prototype to support a community (w support)
	- Build tiny DIY community
- RAISE OR BOOTSTRAP


##### APPROACH
- -> CMKS Feedback -> If no traction, then hobby
- AF-lifestyle 


##### DECISION TREE
**BIG Q**: am I going to have a real DAN-IDEA bat swing?
- YES: 	 then stay with AF and make it happen in 2021-2022
			very risky.  50/50??  and need to really focus on it.
- NO: 


ANSWERED QUESTIONS:
**CHILL Q**: CAN I CHILL w. <$100K for 5 years?
- YES: 	then maybe do that with AF, looking for a DAN-IDEA to drive
- **NO**:	I am going to need to earn.  cannot use 1/2 years before retire.

**CONSULTING CO Q**: Turn AF towards lifestyle consulting biz in <1 year
- YES: 35%  NO: 65%

**AF BAT SWING Q**: Can we generate AF batswing in <1.5 years
- YES: 40%  NO: 60%.    ?????? 

Q: GOING TO TAKE RISK ON AF or JASON
- AF:  	well chances are 50/50 (or less)
- JASON:	then leap for Jason and leave in 12mo to Google (or AF) if it sucks


##### CORE IDEAS
- **CMKS** - Compo-Minimal Knowlege Systems
- **Knowledge Mgt** - 


##### Framing to Jason
- The Good: 
	Solid-team, specific-strong-idea, 
	My role is primarily technical leadership in area w/ significant experience
- The Bad:
	On path to towards impact projects.
	Time delay to cash


##### THINKING
CMKS -- Greatest driver is CMKS.  
- 50/50 chance I could shoot for a real business there.
- If I don't do it now, likely never will
JASON
- 4 years output should be much greater than $4M @ 15% = $30M sale = $10M/yr ~10K customers
- If I leave @ 4yr my 15% is in jeopardy
- Cannot Feriss this
- Will require landing multiple markets or massively dominating one.
AF

FERISS
- Business that is stable and throws off $1M profit is lifestyle
- Needs > $3M revenue


JASON OR GOOGLE AT EMOTIONAL LEVEL
?? Jason?
++ Potentially Greater Control
-- Money could feel weird, but after year 2 if >150K and thriving bus, then ok


JASON OR AF lite + on side driving towards DAN-IDEA
AF-lite (but not so likely to work)
-- 30-40% chance that we can figure out strong consulting model
-- Letting me earn >200K @ 15/hrs week


JASON VS AF PRODUCT
AF-product
SOLID PRODUCT:  < 50/50
-- feels so much safter to be in control the business
-- will need hard work for several of years, but if aiming lifestyle then
   maybe some relaxing.  maybe.
IFFY PRODUCT:
-- Better to go with Jason, at least I am focused on tech & cool tech

_

