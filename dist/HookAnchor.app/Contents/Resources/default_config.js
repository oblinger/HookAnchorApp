...
module.exports = {
  ...
  action_1pass: function(ctx) {
    ...
    try {
      ...
      shell_sync("/bin/sleep 1.0");  // Wait for 1Password to search and show results
  ...
  action_activate_tmux: function(ctx) {
    const { log, shell, shellWithExitCode, error, file_exists, detailed_log } = ctx.builtins;
    const fullPath = ctx.arg;
    
    detailed_log("TMUX_DEBUG", "==================== TMUX ACTIVATION START ====================");
    detailed_log("TMUX_DEBUG", `Line 686: Received arg: '${fullPath}'`);
    detailed_log("TMUX_DEBUG", `Line 687: Type of arg: ${typeof fullPath}`);
    
    if (!fullPath) {
      detailed_log("TMUX_DEBUG", "Line 689: ERROR - No path provided");
    ...
    if (fullPath.endsWith('.md')) {
      detailed_log("TMUX_DEBUG", "Line 696: Path ends with .md, extracting parent directory");
      // It's a markdown file, get the parent directory
      folder_path = fullPath.substring(0, fullPath.lastIndexOf('/'));
      detailed_log("TMUX_DEBUG", `Line 697: Extracted folder_path: '${folder_path}'`);
    } else {
      detailed_log("TMUX_DEBUG", "Line 700: Path is already a folder");
  ...
  // REMOVED: action_activate_anchor - old complex TMUX logic
  // The complex anchor activation with TMUX is now a separate action (Cmd+A)
  /*
  action_activate_anchor_OLD: function(ctx) {
    const { log, shell, shell_sync, shellWithExitCode, open_folder, change_directory, file_exists, joinPath, 
            getObsidianVaultPath, getObsidianVault, encodeURIComponent, spawnDetached } = ctx.builtins;
    const fullPath = ctx.arg;
    
    detailed_log("ACTIVATE_ANCHOR", `Starting activation for: ${fullPath}`);
    
    // Determine if this is a file or folder path
    let anchorDir;
    if (fullPath.endsWith('.md')) {
      // It's a markdown file, get the parent directory
      anchorDir = fullPath.substring(0, fullPath.lastIndexOf('/'));
    } else {
      // It's already a directory
      anchorDir = fullPath;
    }
    
    detailed_log("ACTIVATE_ANCHOR", `Anchor directory: ${anchorDir}`);
    
    // 1. Open the folder in Finder
    try {
      open_folder(anchorDir);
      detailed_log("ACTIVATE_ANCHOR", `Opened folder in Finder: ${anchorDir}`);
    } catch (e) {
      detailed_log("ACTIVATE_ANCHOR", `Failed to open folder: ${e}`);
    }
    
    // 2. Open in Obsidian (if it's in the vault)
    const vaultPath = getObsidianVaultPath();
    if (anchorDir.startsWith(vaultPath)) {
      try {
        const relativePath = anchorDir.substring(vaultPath.length + 1);
        const obsidianURI = `obsidian://open?vault=${getObsidianVault()}&file=${encodeURIComponent(relativePath)}`;
        shell(`open "${obsidianURI}"`);
        detailed_log("ACTIVATE_ANCHOR", `Opened in Obsidian: ${relativePath}`);
      } catch (e) {
        detailed_log("ACTIVATE_ANCHOR", `Failed to open in Obsidian: ${e}`);
      }
    }
    ...
    const tmuxpPath = joinPath(anchorDir, ".tmuxp.yaml");
    if (file_exists(tmuxpPath)) {
      detailed_log("ACTIVATE_ANCHOR", `Found .tmuxp.yaml at ${tmuxpPath}`);
      
      // Change to the anchor directory
      change_directory(anchorDir);
      
      const folderName = anchorDir.split('/').pop();
      detailed_log("ACTIVATE_ANCHOR", `Checking/creating tmux session '${folderName}'`);
      ...
      try {
        ...
        // Create session if it doesn't exist
        if (!existingSessions.includes(folderName)) {
          detailed_log("ACTIVATE_ANCHOR", `Creating new tmux session '${folderName}'`);
          const tmuxpResult = shell_sync(`/opt/homebrew/bin/tmuxp load "${tmuxpPath}" -d`);
          detailed_log("ACTIVATE_ANCHOR", `tmuxp load result: ${tmuxpResult}`);
          shell_sync("/bin/sleep 0.2");
        } else {
          detailed_log("ACTIVATE_ANCHOR", `Session '${folderName}' already exists`);
        }
        
        // Attach to the session - KEY: Use spawnDetached to avoid blocking!
        // This matches the working pattern from the old tmux_activation.js
        detailed_log("ACTIVATE_ANCHOR", `Attaching to session '${folderName}'...`);
        ...
        try {
          const switchResult = spawnDetached(switchCmd);
          detailed_log("ACTIVATE_ANCHOR", `Sent switch-client command for session '${folderName}': ${switchResult}`);
        } catch (switchError) {
          detailed_log("ACTIVATE_ANCHOR", `Switch-client failed: ${switchError}, trying attach-session`);
          // Fall back to attach-session (works when not inside tmux)
          const attachCmd = `/opt/homebrew/bin/tmux attach-session -t "${folderName}"`;
          const attachResult = spawnDetached(attachCmd);
          detailed_log("ACTIVATE_ANCHOR", `Sent attach-session command for session '${folderName}': ${attachResult}`);
        }
        
        // Wait a moment then activate iTerm2
        shell_sync("/bin/sleep 0.5");
        shell('/usr/bin/osascript -e \'tell application "iTerm2" to activate\'');
        log(`ACTIVATE_ANCHOR: Activated iTerm2`);
      } catch (e) {
        detailed_log("ACTIVATE_ANCHOR", `Error managing tmux session: ${e}`);
      }
    } else {
      log(`ACTIVATE_ANCHOR: No .tmuxp.yaml found, skipping tmux activation`);
      
      // If no tmux profile exists but CLAUDE.md exists, launch Claude Code
      const claudeMdPath = joinPath(anchorDir, "CLAUDE.md");
      if (file_exists(claudeMdPath)) {
        log(`ACTIVATE_ANCHOR: Found CLAUDE.md, launching Claude Code`);
        change_directory(anchorDir);
        shell("/opt/homebrew/bin/claude --continue");
      }
    }
    
    return `Activated anchor: ${anchorDir}`;
  }
  */
};